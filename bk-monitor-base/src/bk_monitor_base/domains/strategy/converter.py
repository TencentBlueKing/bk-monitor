...
class Converter(Protocol):
    @classmethod
    def convert(cls, strategy: Strategy) -> None: ...

    @classmethod
    def restore(cls, strategy: Strategy) -> None: ...


class UptimeCheckConvert(Converter):
    """
    拨测策略转换
    """
    ...
    def convert(cls, strategy: Strategy):
        for item in strategy.items:
            ...
            for algorithm in algorithms:
                if algorithm.type == AlgorithmChoices.PartialNodes:
                    algorithm.type = AlgorithmChoices.Threshold
                    algorithm_config = cast(list[list[dict[str, Any]]], algorithm.config)
                    algorithm.config = [[{"method": "gte", "threshold": algorithm_config[0][0].get("count", 0)}]]
        ...
        for item in strategy.items:
            ...
            for query_config in item.query_configs:
                ...
                if "error_code" in condition_dict:
                    ...
                    error_code: int | list[int] = condition_dict["error_code"]
                ...
                for index, condition_msg in enumerate(query_config.agg_condition):
                    ...
                    if condition_msg["key"] == "task_id" and isinstance(condition_msg["value"], tuple | list):
                        value = cast(tuple[Any, ...] | list[Any], condition_msg["value"])
                        condition_msg["value"] = [str(task_id) for task_id in value]
            ...
            algorithms = item.algorithms
            for algorithm in algorithms:
                if algorithm.type == AlgorithmChoices.Threshold:
                    algorithm.type = AlgorithmChoices.PartialNodes
                    algorithm_config = cast(list[list[dict[str, Any]]], algorithm.config)
                    algorithm.config = {"count": algorithm_config[0][0]["threshold"]}


class CMDBTopoNodeAggConvert(Converter):
    """拓扑层级聚合策略转换
    ...
    def convert(cls, strategy: Strategy):
        ...
        """
        # 未开启数据平台，则直接返回
        bkbase_config = get_config().blueking.bkbase
        if not bkbase_config.enabled:
            return
        ...
        for item in strategy.items:
            ...
            bk_obj_ids: set[str] = {TEMPLATE_MAP.get(i["bk_obj_id"]) or i["bk_obj_id"] for i in target[0][0]["value"]}
            for query_config in item.query_configs:
                if (
                    query_config.data_source_label != DataSourceLabel.BK_MONITOR_COLLECTOR
                    or query_config.data_type_label != DataTypeLabel.TIME_SERIES
                ...
                # 2. 开启计算平台 -> N
                # 3. 是否system库相关数据 -> N
                if not bkbase_config.allow_all_cmdb_level:
                    raise Exception("CMDB动态节点聚合暂不可用(原因：全局配置[allow_all_cmdb_level]未开启)")
                if query_config.result_table_id.startswith("system."):
                    raise Exception(
                        "主机性能指标按CMDB动态节点聚合暂不可用(原因：维度未使用云区域ID + IP，目标又选择了CMDB节点)"
                    )
                ...
                for bk_obj_id in bk_obj_ids:
                    try:
                        result = api.metadata.create_result_table_metric_split(
                            ...
                            operator=strategy.update_user,
                        )
                        query_config.result_table_id = result["table_id"]
                    except Exception as e:
                        if "不可对拆分结果表再次拆分" not in str(e):
                            raise e
...
class CMDBTopoNodeAggConvertWithBKData(Converter):
    """基于计算平台的CMDB预聚合"""
    ...
    def convert(cls, strategy: Strategy):
        ...
        for item in strategy.items:
            ...
            for rt_query in item.query_configs:
                ...
                try:
                    api.metadata.full_cmdb_node_info(
                        bk_tenant_id=bk_biz_id_to_bk_tenant_id(strategy.bk_biz_id), table_id=rt_query.result_table_id
                    )
...
class FakeEventConvert(Converter):
    """
    伪系统事件型策略转换
    """
...
class EventTobeClosedConvert(Converter):
    """
    事件恢复机制最终状态为关闭的配置
    """
    ...
    def convert(cls, strategy: Strategy):
        for query_config in chain(*[item.query_configs for item in strategy.items]):
            if query_config.get_metric_id() not in CLOSE_EVNET_METRIC_IDS:
                break
        else:
...
class ActionOptionConvert(Converter):
    """
    处理套餐部分配置与通知配置对齐
    """
...
def convert_strategy(strategy: Strategy):
    """转换策略"""

    for converter in Converters:
        converter.convert(strategy)


def restore_strategy(strategy: Strategy):
    """恢复策略"""

    for converter in Converters:
        converter.restore(strategy)
