...
@final
class MysqlPluginManager(SQLPluginManager):
    """
    Mysql Job插件管理器
    Repo 必须包含以下文件:
    - job_plugin/job_mysql/OracleUniversalPlugin (Linux X86_64)
    - job_plugin/job_mysql/OracleUniversalPlugin_arm (Linux AARCH64)
    """

    type: ClassVar[str] = "job_mysql"
    ...
    def get_binary_path(self, os_type: OSType) -> str:
        """
        获取二进制文件路径
        Mysql 也可使用OracleUniversalPlugin的二进制进行采集
        """
        try:
