...
from bk_monitor_base.metadata.utils.db import array_group
from bk_monitor_base.metadata.utils.es_tools import (
    get_client,
    get_cluster_disk_size,
    get_value_if_not_none,
)
from bk_monitor_base.metadata.utils.time_tools import biz2utc_str
...
class EsSnapshot(models.Model):
    ...
    def _lock_snapshot_scope(cls, table_ids: list, bk_tenant_id: str):
        ...
        locked_snapshots = (
            ...
            .filter(table_id__in=unique_table_ids, bk_tenant_id=bk_tenant_id)
            .order_by("id")
        )
    ...
    def _lock_and_validate_create_snapshot(
        cls, table_ids: list, bk_tenant_id: str, target_snapshot_repository_name: str, status: str | None = None
    ):
        """带锁的校验方法"""
        from bk_monitor_base.metadata.models import ESStorage
        ...
        es_storages = (
            ESStorage.objects.select_for_update()
            .filter(
        ...
        for table_id in table_ids:
            ...
            if status != cls.ES_RUNNING_STATUS:
                continue
            has_running = any(s.status == cls.ES_RUNNING_STATUS for s in snapshots_by_table[table_id])
            if has_running:
    ...
    def create_snapshot(
        ...
        # 使用带锁的校验
        cls._lock_and_validate_create_snapshot([table_id], bk_tenant_id, target_snapshot_repository_name, status)
    ...
    def bulk_create_snapshot(
        ...
        # 使用带锁的校验
        cls._lock_and_validate_create_snapshot(unique_table_ids, bk_tenant_id, target_snapshot_repository_name, status)
    ...
    def modify_snapshot(
        ...
        if (
            ...
            and obj.status != cls.ES_RUNNING_STATUS
            and cls.has_running_snapshot(obj.table_id, obj.bk_tenant_id, exclude_id=obj.id)
        ):
    ...
    def bulk_modify_snapshot(
        ...
        if status == cls.ES_RUNNING_STATUS:
            ...
            other_running = (
                cls.objects.filter(
                    table_id__in=unique_table_ids, bk_tenant_id=bk_tenant_id, status=cls.ES_RUNNING_STATUS
                )
                .exclude(id__in=objs.values_list("id", flat=True))
                .exists()
            )
    ...
    def delete_snapshot(
        ...
        """
        from bk_monitor_base.metadata.task.tasks import delete_es_result_table_snapshot
    ...
    def retry_snapshot(
        ...
        from bk_monitor_base.metadata.task.tasks import retry_es_result_table_snapshot
...
class EsSnapshotIndice(models.Model):
    ...
    def all_doc_count_and_store_size(cls, bk_tenant_id: str, table_ids: list[str], repository_names: list[str]):
        ...
        agg_result = [
            {**item, "table_id_repository_name": (item["table_id"], item["repository_name"])} for item in agg_result
        ]
...
class EsSnapshotRestore(models.Model):
    ...
    def create_restore(
        ...
        from bk_monitor_base.metadata.task.tasks import restore_result_table_snapshot
    ...
    def delete_restore(cls, bk_tenant_id: str, restore_id, operator, is_sync: bool | None = False):
        ...
        from bk_monitor_base.metadata.task.tasks import delete_restore_indices
    ...
    def batch_get_indices(cls, bk_tenant_id: str, restore_ids: list):
        ...
        for (storage_cluster_id, bk_tenant_id), restores in storage_rt_mappings.items():
            ...
            for restore in restores:
                ...
                indices_details = []
                for _restore_index, complete_info in restore_indices.items():
