...
from bk_monitor_base.metadata.models.common import BaseModelWithTime
from bk_monitor_base.metadata.models.record_rule import utils
from bk_monitor_base.metadata.models.record_rule.constants import (
    DEFAULT_EVALUATION_INTERVAL,
    DEFAULT_RULE_TYPE,
    BkDataFlowStatus,
)
from bk_monitor_base.metadata.models.space.ds_rt import get_space_table_id_data_id
from bk_monitor_base.metadata.utils.dataflow_auth import batch_add_permission
from bk_monitor_base.metadata.utils.time_format import parse_duration
...
class RecordRule(BaseModelWithTime):
    ...
    rule_config = models.JSONField("原始规则配置", null=True, blank=True)
    bk_sql_config = models.JSONField("转换后的SQL配置", null=True, blank=True)
    rule_metrics = models.JSONField("指标转换信息", default=dict)
    src_vm_table_ids = models.JSONField("源数据VM结果表列表", default=list)
    ...
    @classmethod
    def get_src_table_ids(cls, space_type: str, space_id: str, metrics: list[str] | set[str]) -> list[str]:
        """获取源结果表列表"""
        ...
        begin_time = tz_now() - datetime.timedelta(seconds=settings.metadata.time_series_metric_expired_seconds)
        group_ids = set(
    ...
    def get_dst_table_id(cls, table_id: str) -> str:
        """获取要写入的结果表"""
        return f"{settings.metadata.default_bkdata_biz_id}_{utils.compose_rule_table_id(table_id)}"
...
class ResultTableFlow(BaseModelWithTime):
    ...
    config = models.JSONField("计算配置", null=True, blank=True)
    ...
    @classmethod
    def compose_source_node(cls, vm_table_ids: list) -> list[dict[str, Any]]:
        """
        组装计算配置
            ...
            nodes.append(
                {
                    ...
                    "bk_biz_name": f"{settings.metadata.default_bkdata_biz_id}",
    ...
    @classmethod
    def compose_process_node(
        cls, bk_tenant_id: str, table_id: str, vm_table_ids: list[str], waiting_time: int = 30
    ) -> dict[str, Any]:
        """
        组装计算节点配置
    ...
    def create_flow(
        ...
        # 组装参数
        req_data = {
            "project_id": settings.metadata.bk_data_record_rule_project_id,
            "flow_name": cls.compose_flow_name(table_id),
        }

        # 检测并授权结果表
        if not batch_add_permission(
            settings.metadata.bk_data_record_rule_project_id,
            settings.metadata.bk_data_bk_biz_id,
            rule_obj.src_vm_table_ids,
        ):
