...
from bk_monitor_base.metadata.models.result_table import (
    ResultTable,
    ResultTableField,
    ResultTableOption,
)
...
class TimeSeriesGroup(CustomGroupBase):
    ...
    @classmethod
    def get_scope_name_from_group_key(
        cls, group_key: str, metric_group_dimensions: list[dict[str, Any]] | None = None
    ) -> str:
        """
    ...
    @staticmethod
    def make_table_id(
        bk_biz_id: int, bk_data_id: int, table_name: str | None = None, bk_tenant_id: str = DEFAULT_TENANT_ID
    ) -> str:
        if settings.blueking.enable_multi_tenancy:  # 若启用多租户模式,则在结果表前拼接租户ID
    ...
    def bulk_refresh_rt_fields(self, table_id: str, metric_info: list):
        ...
        aggregated_metrics: dict[str, dict[str, Any]] = defaultdict(lambda: {"tag_value_list": {}, "tag_list": []})
    ...
    def get_metrics_from_redis(self, expired_time: int | None = settings.metadata.time_series_metric_expired_seconds):
        """从 redis 中获取数据
        ...
        custom_metrics_key = f"{settings.metadata.metrics_key_prefix}{self.bk_data_id}"
        metric_dimensions_key = f"{settings.metadata.metric_dimensions_key_prefix}{self.bk_data_id}"

        now_time = tz_now()
        fetch_step = settings.metadata.max_metrics_fetch_step
    ...
    def update_time_series_metrics(self) -> bool:
        ...
        metrics_info = self.get_metrics_from_redis(
            expired_time=settings.metadata.fetch_time_series_metric_interval_seconds
        )
        # 如果为空，直接返回
        if not metrics_info:
            return False
    ...
    def get_metric_info_list_with_label(self, dimension_name: str, dimension_value: str):
        ...
        # 获取过期分界线
        last = datetime.datetime.now() - datetime.timedelta(
            seconds=settings.metadata.time_series_metric_expired_seconds
        )
        # 查找过期时间以前的数据
...
class TimeSeriesScope(models.Model):
    ...
    DimensionConfigFields: list[str] = [
        "alias",  # 字段别名
        "common",  # 常用维度
        "hidden",  # 显隐
    ]
    ...
    # 创建来源选项
    CREATE_FROM_DATA: str = "data"  # 数据自动创建
    CREATE_FROM_USER: str = "user"  # 用户手动创建
    CREATE_FROM_DEFAULT: str = "default"  # 默认分组
    CREATE_FROM_CHOICES = [
    ...
    scope_name = models.CharField(verbose_name="指标分组名", max_length=255, db_collation=db_collation)
    ...
    def _collect_metrics_and_dimensions(cls, group, metric_info_list: list) -> tuple:
        ...
        scope_id_to_name: dict[int, str | None] = {scope.id: scope.scope_name for scope in all_scopes}
...
class TimeSeriesMetric(models.Model):
    ...
    DISABLE_SCOPE_ID: int = 0  # 禁用分组的 id，实际上不存在该分组记录
    ...
    field_scope = models.CharField(
        verbose_name="指标字段数据分组名", default=DEFAULT_DATA_SCOPE_NAME, max_length=255, db_collation=db_collation
    )
    field_name = models.CharField(verbose_name="指标字段名称", max_length=255, db_collation=db_collation)
    tag_list = models.JSONField(verbose_name="Tag列表", default=[])
    ...
    def _bulk_update_metrics(
        ...
        for obj in qs_objs:
            ...
            if (
                last_modify_time - obj.last_modify_time
            ).seconds >= settings.metadata.time_series_metric_expired_seconds:
        ...
        logger.info(
            "white list disabled metric: %s, group_id: %s", json.dumps(list(white_list_disabled_metric)), group_id
        )
...
class TimeSeriesTag(models.Model):
    ...
    values = models.JSONField(verbose_name="维度值列表", default=list)
