...
from bk_monitor_base.metadata.models.constants import LOG_REPORT_MAX_QPS
from bk_monitor_base.metadata.models.custom_report.log import LogGroup
...
if TYPE_CHECKING:
    from bk_monitor_base.metadata.models.custom_report.event import EventGroup
    from bk_monitor_base.metadata.models.custom_report.time_series import TimeSeriesGroup
...
class CustomReportSubscription(models.Model):
    ...
    @classmethod
    def get_protocol(cls, bk_data_id: int) -> str:
        from bk_monitor_base.metadata.models.custom_report.time_series import TimeSeriesGroup
        ...
        try:
            ...
            return "json"
        try:
            # todo 底座依赖saas不合理，后续优化删除这段逻辑，在TimeSeriesGroup表增加protocol字段
            ts_info: dict[str, Any] = api.bk_monitorv3.custom_time_series_detail(
    ...
    def get_custom_config(
        cls,
        query_set: models.QuerySet["EventGroup"] | models.QuerySet["TimeSeriesGroup"],
            ...
            if protocol == "json":
                ...
                try:
                    if group.custom_group_id in disabled_ts_group_ids:
                        continue
                    # prometheus格式token直接构造
                    access_token: str = transform_data_id_to_token(
                        metric_data_id=r["bk_data_id"], bk_biz_id=r["bk_biz_id"], app_name=group.time_series_group_name
                    )
                except Exception:
                    logger.warning(f"[{r['bk_data_id']}],[{group.custom_group_id}] make token failed", exc_info=True)
                    continue

                # prometheus格式: bk-collector-application.conf
                item = {
                    "bk_data_token": access_token,
    ...
    def _refresh_k8s_custom_config_by_biz(
        ...
        if settings.metadata.custom_report_default_deploy_cluster:
            # 补充中心化集群
            for cluster_id in settings.metadata.custom_report_default_deploy_cluster:
    ...
    def refresh_collector_custom_conf(cls, bk_tenant_id: str, bk_biz_id: int | None = None, op_type: str = "add"):
        ...
        if bk_biz_id is None:
            all_bk_biz_ids = {b.bk_biz_id for b in api.cmdb.search_business(bk_tenant_id=bk_tenant_id)[1]}
            bk_biz_ids = {
                bk_biz_id
    ...
    def create_or_update_config(cls, bk_tenant_id: str, bk_biz_id: int, subscription_params, bk_data_id=0):
        ...
        # 如果订阅存在，则更新订阅
        if sub_config_obj:
            logger.info("data_id %s subscription task already exists.", bk_data_id)
            # 更新订阅巡检开启
            api.node_man.switch_subscription(
                bk_tenant_id=bk_tenant_id,
                subscription_id=sub_config_obj.subscription_id,
                action="enable",
            )
            ...
            if old_subscription_params_md5 != new_subscription_params_md5:
                ...
                result = api.node_man.update_subscription(bk_tenant_id=bk_tenant_id, params=subscription_params)
...
class LogSubscriptionConfig(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["bk_biz_id", "log_name"]),
        ]
    ...
    def refresh(cls, log_group: "LogGroup") -> None:
        ...
        Refresh Config
        """
        from bk_monitor_base.metadata.utils.bk_collector_config import BkCollectorConfig
    ...
    @classmethod
    def get_log_config(cls, log_group: "LogGroup") -> dict[str, Any]:
        # 数据库：
        # 小于 -1 表示全部限制，-1 是默认值 MAX_DATA_ID_THROUGHPUT，0 表示不限制，大于 0 为实际的 qps
        # 采集器：
        # 小于 0 表示全部限制, 0 表示不限制，大于 0 为实际的 qps
    ...
    def deploy(cls, bk_tenant_id: str, log_group: "LogGroup", log_config: dict, bk_host_ids: list[int]) -> None:
        ...
        if sub_config_obj:
            ...
            result = api.node_man.create_subscription(bk_tenant_id=bk_tenant_id, params=subscription_params)
