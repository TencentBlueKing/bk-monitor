...
from bk_monitor_base.metadata.models.data_link import DataLink
...
from bk_monitor_base.metadata.models.vm.bk_data import BkDataAccessor, access_vm
from bk_monitor_base.metadata.models.vm.config import BkDataStorageWithDataID
from bk_monitor_base.metadata.models.vm.constants import (
    ACCESS_DATA_LINK_FAILURE_STATUS,
    ACCESS_DATA_LINK_SUCCESS_STATUS,
    BKDATA_NS_TIMESTAMP_DATA_ID_LIST,
    TimestampLen,
)
from bk_monitor_base.metadata.utils.tenant import get_tenant_datalink_biz_id, get_tenant_default_biz_id

logger = logging.getLogger("metadata")


def refine_bkdata_kafka_info(bk_tenant_id: str):
    from bk_monitor_base.metadata.models import ClusterInfo
...
def access_bkdata(bk_tenant_id: str, bk_biz_id: int, table_id: str, data_id: int):
    ...
    from bk_monitor_base.metadata.models import AccessVMRecord, KafkaStorage, Space, SpaceVMInfo
    ...
    timestamp_len: int | None = get_timestamp_len(data_id)
    try:
    ...
    # 3. 当出现异常时，记录对应日志
    if (
        settings.metadata.bcs_data_convergence_config.get("is_enabled")
        and settings.metadata.bcs_data_convergence_config.get("k8s_metric_rt")
        and settings.metadata.bcs_data_convergence_config.get("custom_metric_rt")
        and bcs_cluster_id
    ):
        try:
            ...
            clean_data["result_table_id"] = (
                settings.metadata.bcs_data_convergence_config["k8s_metric_rt"]
                if data_type == AccessVMRecord.BCS_CLUSTER_K8S
                else settings.metadata.bcs_data_convergence_config["custom_metric_rt"]
            )
            clean_data["processing_id"] = data_name_and_dp_id["dp_id"]
            # 创建清洗
            api.bkdata.databus_cleans(bk_tenant_id=bk_tenant_id, params=clean_data)
            # 启动
...
def access_vm_by_kafka(
    bk_tenant_id: str, table_id: str, raw_data_name: str, vm_cluster_name: str, timestamp_len: int | None
) -> dict[str, Any]:
    """通过 kafka 配置接入 vm"""
    from bk_monitor_base.metadata.models import BkDataStorage, KafkaStorage, ResultTable
    ...
    try:
        bkbase_result_table_id = api.bkdata.databus_cleans(bk_tenant_id=bk_tenant_id, params=clean_data)[
            "result_table_id"
        ]
        # 启动
        api.bkdata.start_databus_cleans(
            bk_tenant_id=bk_tenant_id, result_table_id=bkbase_result_table_id, storages=["kafka"]
        )
    except Exception as e:
        logger.error(
    ...
    try:
        ...
        api.bkdata.create_data_storages(bk_tenant_id=bk_tenant_id, **storage_params)
        return {
...
def get_data_type_cluster(data_id: int) -> dict:
    from bk_monitor_base.metadata.models import AccessVMRecord, BCSClusterInfo
...
def report_metadata_data_link_status_info(data_link_name: str, biz_id: str, kind: str, status: str):
    ...
    try:
        ...
        logger.info(
            "try to report metadata data link status info,data_link_name->[%s], status_number->[%s]",
            data_link_name,
            status_number,
        )
...
def get_vm_cluster_id_name(
    ...
    3. 如果没有接入过，则需要使用默认值
    """
    from bk_monitor_base.metadata.models import ClusterInfo, SpaceVMInfo
...
def get_bkbase_data_name_and_topic(table_id: str) -> dict:
    ...
    return {"data_name": vm_name, "topic_name": f"{vm_name}{settings.metadata.default_bkdata_biz_id}"}
...
def get_bcs_convergence_data_name_and_dp_id(table_id: str) -> dict:
    ...
    name = f"{table_id.replace('-', '_').replace('.', '_').replace('__', '_')[-40:]}"
    # NOTE: 清洗结果表不能出现双下划线
    return {"data_name": f"dp_{name}", "dp_id": f"{settings.metadata.default_bkdata_biz_id}_{name}_dp_metric_all"}


def get_timestamp_len(data_id: int | None = None, etl_config: str | None = None) -> int | None:
...
def access_v2_bkdata_vm(bk_tenant_id: str, bk_biz_id: int, table_id: str, data_id: int, force_update: bool = False):
    ...
    from bk_monitor_base.metadata.models import AccessVMRecord, DataSource, Space, SpaceVMInfo
...
def create_bkbase_data_link(
    ...
    namespace: str | None = settings.metadata.default_vm_data_link_namespace,
...
def create_fed_bkbase_data_link(
    ...
    创建联邦集群汇聚链路（子集群->代理集群）
    """
    from bk_monitor_base.metadata.models import BcsFederalClusterInfo
    from bk_monitor_base.metadata.models.data_link.utils import is_k8s_metric_data_id
