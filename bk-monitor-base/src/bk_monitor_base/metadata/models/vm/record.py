...
from bk_monitor_base.metadata.models.vm.managers import SpaceVMInfoManager
...
class AccessVMRecord(models.Model):
    ...
    storage_cluster_id = models.IntegerField(
        "对接使用的storage域名", null=True, blank=True, help_text="对接使用的集群ID"
    )
    # vm 对应的集群ID
    vm_cluster_id = models.IntegerField(
        "集群ID", null=True, blank=True, help_text="因为查询 vm 集群已经统一，不需要再单独记录"
    )
...
class SpaceVMInfo(BaseModel):
    ...
    objects: ClassVar[SpaceVMInfoManager] = SpaceVMInfoManager()
