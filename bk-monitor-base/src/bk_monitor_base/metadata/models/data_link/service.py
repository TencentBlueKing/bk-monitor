...
from bk_monitor_base.metadata.models.data_link.constants import DataLinkKind, DataLinkResourceStatus
...
def apply_data_id_v2(
    ...
    data_name: str,
    bk_biz_id: int,
    namespace: str = settings.metadata.default_vm_data_link_namespace,
    is_base: bool = False,
...
def get_data_link_component_status(
    ...
    try:
        ...
        logger.error(
            "get_data_link_component_status: get component status failed,kind->[%s],name->[%s],namespace->[%s],error->[%s]",
            kind,
            component_name,
            namespace,
            e.__cause__,
