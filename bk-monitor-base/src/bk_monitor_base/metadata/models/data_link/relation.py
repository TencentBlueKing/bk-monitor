...
from bk_monitor_base.metadata.models import AccessVMRecord, DataSourceResultTable
from bk_monitor_base.metadata.models.data_link.constants import DataLinkKind
...
def rebuild_databus_relation(databus: DataBusConfig, dry_run: bool = True) -> DataLink | dict[str, object] | None:
    ...
    for instance in sink_instances:
        if instance.data_link_name:
            logger.error(
                "rebuild_databus_relation: databus->[%s] sink component kind->[%s] name->[%s] already has data_link_name->[%s], conflict detected, skip",
                databus_name,
                instance.kind,
                instance.name,
    ...
    for instance in sink_instances:
        ...
        if not rt_name:
            logger.warning(
                "rebuild_databus_relation: databus->[%s] binding kind->[%s] name->[%s] has empty bkbase_result_table_name, skip",
                databus_name,
                instance.kind,
                instance.name,
            )
    ...
    for rt in rt_instances:
        if rt.data_link_name:
            logger.error(
                "rebuild_databus_relation: databus->[%s] ResultTableConfig name->[%s] already has data_link_name->[%s], conflict detected, skip",
                databus_name,
                rt.name,
                rt.data_link_name,
            )
    ...
    logger.info(
        "rebuild_databus_relation: databus->[%s] relation rebuilt successfully, strategy->[%s], table_ids->[%s], created->[%s]",
        databus_name,
        strategy,
        table_ids,
        created,
...
def rebuild_bkbase_v4_datalink_relation(bk_tenant_id: str, namespace: str, dry_run: bool = True) -> list[dict] | None:
    ...
    logger.info(
        "rebuild_bkbase_v4_datalink_relation: found %d DataBus(es) with empty data_link_name for bk_tenant_id->[%s] namespace->[%s] dry_run->[%s]",
        total,
        bk_tenant_id,
        namespace,
        dry_run,
