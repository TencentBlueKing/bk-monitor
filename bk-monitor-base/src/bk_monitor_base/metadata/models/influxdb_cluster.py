...
from bk_monitor_base.metadata.utils import consul_tools, go_time
...
class InfluxDBTool:
    ...
    @classmethod
    def clean_redis_config(cls, key: str, exist_list: list[str]):
        """清理不存在的数据"""
...
class InfluxDBHostInfo(models.Model, InfluxDBTool):
    ...
    def import_data(cls, data):
        ...
        for item in items:
            ...
            if valid_message:
                # 脱敏
                item["password"] = "xxxx"
                raise Exception(f"host->{item} check valid failed,reason->{valid_message}")
    ...
    def update_default_rp(self, refresh_databases):
        ...
        duration_str = f"{settings.metadata.ts_data_saved_days}d"
...
class InfluxDBProxyStorage(BaseModel, InfluxDBTool):
    ...
    def clean_consul(cls):
        ...
        logger.warning("delete consul path: %s", json.dumps(list(diff_service_names)))
