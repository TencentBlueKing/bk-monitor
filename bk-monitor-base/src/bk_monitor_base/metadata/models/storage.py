...
from bk_monitor_base.infras.third_party_api.bkdata.api import DataBusCleansParams
from bk_monitor_base.metadata import config
from bk_monitor_base.metadata.config import settings
from bk_monitor_base.metadata.dataflow.constants import BK_DATA_RAW_TABLE_SUFFIX
from bk_monitor_base.metadata.dataflow.task.cmdblevel import CMDBPrepareAggregateTask
from bk_monitor_base.metadata.dataflow.task.downsample import StatisticTask
from bk_monitor_base.metadata.dataflow.task.filtertime import FilterUnknownTimeTask
from bk_monitor_base.metadata.models import constants
from bk_monitor_base.metadata.models.influxdb_cluster import InfluxDBTool
from bk_monitor_base.metadata.utils import consul_tools, es_tools, go_time, influxdb_tools
from bk_monitor_base.metadata.utils import dataflow_auth as auth
from bk_monitor_base.metadata.utils.es_curator import IndexList
...
from bk_monitor_base.metadata.utils.tools import gen_bk_data_rt_id_without_biz_id, to_bk_data_rt_id
...
if TYPE_CHECKING:
    from bk_monitor_base.metadata.models.custom_report import EventGroup
    from bk_monitor_base.metadata.models.result_table import ResultTable, ResultTableField
else:
...
class ClusterInfo(models.Model):
    ...
    default_settings = models.JSONField("集群的默认配置", default=dict, null=True)
    ...
    def refresh_consul_storage_config(cls):
        ...
        for cluster_id, storage_info in list(refresh_dict.items()):
            ...
            logger.debug(f"consul path->[{consul_path}] is refresh with value->[{value}] success.")
    ...
    def create_cluster(
        ...
        """
        from bk_monitor_base.metadata.models.data_link.data_link_configs import ClusterConfig
...
class InfluxDBStorage(models.Model, StorageResultTable, InfluxDBTool):
    ...
    def push_redis_data(self, is_publish: bool = True):
        ...
        # NOTE: 需要观察 db 的负载
        from bk_monitor_base.metadata.models.vm.record import AccessVMRecord
    ...
    def get_tag_values(self, tag_name) -> list[str]:
        proxy = influxdb_tools.InfluxDBSchemaProxy(
            host=self.storage_cluster.domain_name,
            port=self.storage_cluster.port,
            database=self.database,
    ...
    def refresh_additional_info_for_unify_query(cls):
        ...
        TODO: 待移除
        """
        from bk_monitor_base.metadata.models import InfluxDBProxyStorage
    ...
    def _get_table_id_bcs_cluster_id(cls) -> dict:
        """获取结果表和 BCS 集群的关系"""
        from bk_monitor_base.metadata.models import BCSClusterInfo
        from bk_monitor_base.metadata.models.data_source import DataSourceResultTable
    ...
    def _get_table_id_access_vm_data(cls, table_ids: list[str]) -> dict:
        """获取结果表和集群 ID 的关系"""
        from bk_monitor_base.metadata.models import AccessVMRecord
    ...
    def _get_data_source_by_table_ids(cls, table_ids: list[str]) -> dict[str, dict]:
        """获取结果表对应的数据源信息"""
        from bk_monitor_base.metadata.models import DataSource, DataSourceResultTable
    ...
    def _get_biz_id_by_table_ids(cls, table_id_map: dict, table_id_data_source_map: dict):
        ...
        from bk_monitor_base.metadata.models import EventGroup, TimeSeriesGroup
    ...
    def _get_measurement_type(cls, table_id_map: dict) -> dict:
        """获取 measurement 类型"""
        from bk_monitor_base.metadata.models.space.ds_rt import get_measurement_type_by_table_id
...
class KafkaStorage(models.Model, StorageResultTable):
    ...
    def create_table(
        ...
        **kwargs,
        ...
        # 0. 判断是否需要使用默认集群信息
        if storage_cluster_id is None:
            if settings.metadata.default_kafka_storage_cluster_id is not None:
                storage_cluster_id = settings.metadata.default_kafka_storage_cluster_id
            else:
                storage_cluster_id = ClusterInfo.objects.get(
...
class ESStorage(models.Model, StorageResultTable):
    ...
    # }
    warm_phase_settings = models.JSONField("暖数据配置", default=dict)
    # 时区设置 取值[12]-[-12], 默认0时区
    ...
    def create_table(
        ...
        **kwargs,
        ...
        test_str = datetime.datetime.now(datetime.UTC).strftime(date_format)
    ...
    def index_body(self):
        ...
        :return: dict, 可以直接
        """
        from bk_monitor_base.metadata.models import ResultTableField, ResultTableFieldOption
    ...
    def now(self):
        return arrow.utcnow().replace(hours=self.time_zone).datetime + datetime.timedelta(
            hours=settings.metadata.es_storage_offset_hours
        )
    ...
    def is_index_ready(self, index_name: str) -> bool:
        ...
        try:
            ...
            health = self.es_client.cluster.health(
                ...
                request_timeout=settings.metadata.metadata_request_es_timeout_seconds,
            )
            logger.info(
    ...
    def _should_create_index(self, force_rotate: bool = False) -> bool:
        """
        是否需要创建新索引
        ...
        # avoid index always not split, it not be allocated to cold node
    ...
    def _update_aliases_with_retry(self, actions, new_index_name, force_rotate: bool = False):
        ...
        try:
            response = self.es_client.indices.update_aliases(
                body={"actions": actions}, request_timeout=settings.metadata.metadata_request_es_timeout_seconds
            )
            logger.info(
    ...
    def group_expired_alias(self, alias_list, expired_days, need_delay_delete_alias: bool | None = True):
        ...
        expired_datetime_point: int = self.now - datetime.timedelta(days=expired_days)
        ...
        for index_name, alias_info in alias_list.items():
            ...
            for alias_name in alias_info["aliases"]:
                ...
                if not datetime_str:
                    ...
                    if settings.metadata.es_retain_invalid_alias:
                        # 保留不合法的别名，将该别名视为未过期
                ...
                try:
                    ...
                    logger.error(
                        "table_id->[%s] got index->[%s] with datetime_str->[%s] which is not match date_format->[%s], something go wrong?",
                        self.table_id,
                        index_name,
                        datetime_str,
                        self.date_format,
    ...
    def can_delete_snapshot(self):
        ...
        es_snapshot: EsSnapshot | None = EsSnapshot.objects.filter(self.running_snapshot_query).first()
    ...
    def can_delete(self) -> bool:
        ...
        if self.expired_index():
            if not current_snapshot_info["datetime"]:
                return False
            if current_snapshot_info["datetime"].day != datetime.datetime.now(datetime.UTC).day:
                return False
    ...
    def create_target_index_meta_info(self, index_name, snapshot_name):
        create_obj = {
            ...
            ).astimezone(timezone(django_settings.TIME_ZONE)),
...
class BkDataStorage(models.Model, StorageResultTable):
    ...
    def data_source(self):
        ...
        对应的监控平台数据源ID
        """
        from bk_monitor_base.metadata.models import DataSourceResultTable
    ...
    def create_databus_clean(self, result_table):
        ...
        broker_url = settings.metadata.bk_data_kafka_broker_url
        if domain and port:
        ...
        raw_data_name = "{}_{}".format(
            settings.metadata.bk_data_rt_id_prefix, result_table.table_id.replace(".", "__")
        )[-50:]
        params = {
            "bk_tenant_id": self.bk_tenant_id,
            "data_scenario": "queue",
            "bk_biz_id": settings.metadata.bk_data_bk_biz_id,
            "description": "",
            "access_raw_data": {
                "raw_data_name": raw_data_name,
                "maintainer": settings.metadata.bk_data_project_maintainer,
    ...
    def get_etl_status(self, processing_id):
        ...
        """
        etl_templates = api.bkdata.get_databus_cleans(bk_tenant_id=self.bk_tenant_id, raw_data_id=self.raw_data_id)
    ...
    def check_and_access_bkdata(self):
        ...
        result_table_id = f"{settings.metadata.bk_data_bk_biz_id}_{bk_data_rt_id_without_biz_id}"
        params: DataBusCleansParams = {
        ...
        if not self.etl_json_config:
            # 执行创建操作
            try:
                result = api.bkdata.databus_cleans(bk_tenant_id=self.bk_tenant_id, params=params)
            except Exception:  # noqa
            ...
            if self.etl_json_config != etl_json_config:
                # 执行更新操作
                try:
                    result = api.bkdata.stop_databus_cleans(
                        bk_tenant_id=self.bk_tenant_id, result_table_id=result_table_id, storages=["kafka"]
                    )
                except Exception:  # noqa
                ...
                try:
                    ...
                    result = api.bkdata.update_databus_cleans(bk_tenant_id=self.bk_tenant_id, **params)
                except Exception:  # noqa
        ...
        if etl_status != DataBusStatus.RUNNING:
            for _i in range(3):
                # 启动清洗任务
                try:
                    result = api.bkdata.start_databus_cleans(
                        bk_tenant_id=self.bk_tenant_id, result_table_id=result_table_id, storages=["kafka"]
                    )
                    # 轮训状态，查看是否启动成功
                    for _k in range(10):
            ...
            auth.ensure_has_permission_with_rt_id(
                settings.metadata.bk_data_project_maintainer, result_table_id, int(settings.metadata.bk_data_project_id)
            )
    ...
    def filter_unknown_time_with_rt(self):
        ...
        """
        from bk_monitor_base.metadata.models.result_table import ResultTableField
        ...
        try:
            ...
            for _i in range(60):
                flow_deploy_info = api.bkdata.get_latest_deploy_data_flow(
                    bk_tenant_id=self.bk_tenant_id, flow_id=task.data_flow.flow_id
                )
    ...
    def generate_bk_data_etl_config(self):
        from bk_monitor_base.metadata.models.result_table import ResultTableField
        from bk_monitor_base.metadata.models.vm.constants import TimestampLen
        from bk_monitor_base.metadata.models.vm.utils import get_timestamp_len
    ...
    def full_cmdb_node_info_to_result_table(self):
        if not settings.blueking.bkbase.allow_all_cmdb_level:
            return
        ...
        task = CMDBPrepareAggregateTask(
            rt_id=to_bk_data_rt_id(self.table_id, BK_DATA_RAW_TABLE_SUFFIX),
            agg_interval=0,
            agg_method="",
            metric_field=metric_fields,
            dimension_fields=dimension_fields,
