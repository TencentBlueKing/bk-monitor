...
from bk_monitor_base.metadata.config import db_collation, settings
from bk_monitor_base.metadata.models.bkdata.result_table import BkBaseResultTable
from bk_monitor_base.metadata.models.constants import BULK_CREATE_BATCH_SIZE, DataIdCreatedFromSystem
from bk_monitor_base.metadata.models.data_link.constants import BKBASE_NAMESPACE_BK_MONITOR
from bk_monitor_base.metadata.utils.basic import getitems
...
class ResultTable(models.Model):
    ...
    def real_storage_list(self):
        ...
        """
        result = []
        for _storage_str, real_storage in list(self.REAL_STORAGE_DICT.items()):
            try:
    ...
    @classmethod
    def get_table_id_cutter(
        cls, table_ids: list[str] | set[str], bk_tenant_id: str = DEFAULT_TENANT_ID
    ) -> dict[str, bool]:
        """获取结果表是否禁用切分模块"""
    ...
    def get_related_datasource(self, refresh: bool = False) -> DataSource:
        ...
        if getattr(self, "_related_datasource", None) is not None and not refresh:
            return self._related_datasource
        ...
        self._related_datasource = related_datasource
    ...
    def create_result_table(
        ...
        if target_bk_biz_id != 0:
            try:
                from bk_monitor_base.metadata.models import Space
    ...
    def apply_datalink(self, force_update: bool = False) -> None:
        ...
        from bk_monitor_base.metadata.task.datalink import apply_event_group_datalink, apply_log_datalink
        from bk_monitor_base.metadata.task.tasks import access_bkdata_vm
        ...
        if self.default_storage == ClusterInfo.TYPE_INFLUXDB:
            ...
            if (
                is_v4_datalink_etl_config and settings.metadata.enable_v2_vm_data_link
    ...
    def check_and_create_storage(
        ...
        if self.default_storage == ClusterInfo.TYPE_INFLUXDB and not settings.metadata.enable_influxdb_storage:
            return
    ...
    def modify(
        ...
        if default_storage is not None:
            ...
            if (
                settings.metadata.enable_influxdb_storage
        ...
        for ex_storage_type, ex_storage_config in list(external_storage.items()):
            ...
            for storage_type, _delete_storage in need_delete_storages.items():
    ...
    def set_metric_split(self, cmdb_level, operator):
        ...
        if not settings.blueking.bkbase.allow_all_cmdb_level and data_id < 10000:
    ...
    def to_json_self_only(self) -> dict[str, Any]:
        """
...
class ResultTableField(models.Model):
    ...
    field_name = models.CharField("字段名", max_length=255, db_collation=db_collation)
    ...
    def batch_get_fields(
        ...
        for i in qs:
            ...
            if settings.metadata.enable_consul_lite_mode and is_consul_config:
...
class ESFieldQueryAliasOption(BaseModel):
    ...
    def manage_query_alias_settings(
        ...
        try:
            ...
            logger.error(
                "manage_query_alias_settings: failed to manage alias settings for table_id->[%s], query_alias_settings->[%s], error->[%s]",
                table_id,
                query_alias_settings,
                e,
            )
