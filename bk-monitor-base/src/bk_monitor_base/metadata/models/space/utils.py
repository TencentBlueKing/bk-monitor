...
from bk_monitor_base.metadata.models.bcs import BCSClusterInfo
from bk_monitor_base.metadata.models.constants import BULK_CREATE_BATCH_SIZE
...
def get_space_detail(
    ...
    # 组合返回数据
    detail: dict[str, Any] = space_obj.to_dict()
    # 关联的资源和数据源信息
...
def create_space(
    ...
    try:
        if space_type_id == SpaceTypes.BKSAAS.value:
            authorize_data_id_list(space_type_id, space_id, settings.metadata.bkpaas_authorized_data_id_list)
...
def create_bksaas_space_resource(space_type: str, space_id: str, creator: str):
    ...
    cluster_namespaces = api.bk_paas.get_app_cluster_namespace(bk_tenant_id=DEFAULT_TENANT_ID, app_code=space_id)
...
@atomic(config.DATABASE_CONNECTION_NAME)
def update_space(
    updater: str, space_type_id: str, space_id: str, space_name: str, space_code: str, resources: list[dict[str, Any]]
) -> dict[str, Any]:
    """更新空间信息
...
def disable_space(spaces: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """禁用空间
...
def create_bkcc_spaces(biz_list: list[dict[str, Any]], create_builtin_data_link_delay: bool = True) -> bool:
    """创建业务对应的空间信息
    ...
    :return: 返回 True 或异常
    """
    from bk_monitor_base.metadata.task.tasks import (
    ...
    # 初始化空间内置数据链路
    if settings.metadata.enable_v2_vm_data_link and settings.metadata.enable_space_builtin_data_link:
        for biz in biz_list:
...
def get_valid_bcs_projects(bk_tenant_id: str) -> list:
    """获取可用的 BKCI(BCS) 项目空间"""
    projects = api.bcs.get_projects(bk_tenant_id=bk_tenant_id, kind="k8s")
    # 排除业务ID为 0 的记录，或者绑定的 BKCC 业务 ID 不存在的信息
    bk_biz_id_list = [b.bk_biz_id for b in api.cmdb.search_business(bk_tenant_id=bk_tenant_id)[1]]
    # 返回有效的项目记录
    valid_project_list = []
    for p in projects:
