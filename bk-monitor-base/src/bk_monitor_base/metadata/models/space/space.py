...
from bk_monitor_base.metadata.models.common import BaseModel
...
class SpaceType(BaseModel):
    ...
    dimension_fields = models.JSONField(
        "关键校验字段", help_text="需要的校验的关键字段，格式如['project_id', 'cluster_id', 'namespace']", default=list
    )
...
class Space(BaseModel):
    ...
    space_id = models.CharField("空间 ID", max_length=128, help_text="空间类型下唯一", db_collation=db_collation)
    bk_tenant_id = models.CharField("租户ID", max_length=256, null=True, default="system")
    space_name = models.CharField("空间中文名称", max_length=256, help_text="空间类型下唯一", db_collation=db_collation)
    ...
    def to_dict(self, fields: list | None = None, exclude: list | None = None) -> dict[str, Any]:
        data = {}
...
class SpaceStickyInfo(models.Model):
    ...
    space_uid_list = models.JSONField("置顶空间uid列表", default=list)
