...
from bk_monitor_base.metadata.models.constants import DEFAULT_MEASUREMENT, DataIdCreatedFromSystem
from bk_monitor_base.metadata.models.space import utils
...
from bk_monitor_base.metadata.models.space.ds_rt import (
...
from bk_monitor_base.metadata.utils.db import filter_model_by_in_page, filter_query_set_by_in_page
...
class SpaceTableIDRedis:
    ...
    def _compose_record_rule_table_id_detail(self, bk_tenant_id: str) -> dict[str, dict]:
        """组装预计算结果表的详情"""
        from bk_monitor_base.metadata.models.record_rule.rules import RecordRule
    ...
    def _compose_bcs_space_biz_table_ids(self, space_type: str, space_id: str, bk_tenant_id=DEFAULT_TENANT_ID) -> dict:
        ...
        rts = models.ResultTable.objects.filter(
            ...
            | Q(table_id__in=settings.metadata.bkci_space_access_plugin_list)
        )

        if settings.blueking.enable_multi_tenancy:  # 若开启多租户模式,则这里应该会变成新版1001数据
    ...
    def _compose_bkci_level_table_ids(self, space_type: str, space_id: str, bk_tenant_id=DEFAULT_TENANT_ID) -> dict:
        ...
        for tid in table_ids:
            if tid in settings.metadata.special_rt_route_alias_result_table_list:
    ...
    def _compose_data(
        ...
        for tid in table_ids:
            ...
            if tid.startswith(DBM_1001_TABLE_ID_PREFIX) and space_uid in settings.metadata.access_dbm_rt_space_uid:
    ...
    def _is_need_filter_for_bkcc(
        ...
        if not settings.metadata.is_restrict_ds_belong_space and (
            data_id_detail["space_uid"] == f"{space_type}__{space_id}"
        ):
            return False
    ...
    def _filter_ts_info(self, table_ids: set[str], bk_tenant_id: str = DEFAULT_TENANT_ID) -> dict[str, Any]:
        """根据结果表获取对应的时序数据"""
        if not table_ids:
            return {}
        _filter_data = filter_model_by_in_page(
        ...
        # 根据特性开关决定使用哪种过滤方式
        if settings.metadata.enable_ts_metric_filter_by_is_active:
        ...
        group_id_field_map = {group_id: sorted(field_names) for group_id, field_names in group_id_field_map.items()}
    ...
    def _compose_table_id_fields(self, table_ids: set | None = None, bk_tenant_id: str = DEFAULT_TENANT_ID) -> dict:
        ...
        logger.info("white table_id list: %s", json.dumps(list(white_tables)))
