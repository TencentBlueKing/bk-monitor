...
from bk_monitor_base.infras.constant import DEFAULT_TENANT_ID, DEFAULT_USERNAME
...
from bk_monitor_base.metadata.constants.data_source import DATA_LINK_V3_VERSION_NAME, DATA_LINK_V4_VERSION_NAME
from bk_monitor_base.metadata.models.data_link.constants import BKBASE_NAMESPACE_BK_LOG, BKBASE_NAMESPACE_BK_MONITOR
...
from bk_monitor_base.metadata.utils import consul, consul_tools, hash_util
from bk_monitor_base.metadata.utils.basic import get_space_uid_and_bk_biz_id_by_bk_data_id
from bk_monitor_base.metadata.utils.tenant import get_tenant_datalink_biz_id
...
class DataSource(models.Model):
    ...
    source_system = models.CharField(verbose_name="请求来源的系统", max_length=256, default=settings.blueking.app_code)
    # 是否启用数据源
    is_enable = models.BooleanField(verbose_name="数据源是否启用", default=True)

    transfer_cluster_id = models.CharField(
        verbose_name="transfer集群ID", default=settings.metadata.default_transfer_cluster_id, max_length=50
    )
        ...
        for real_storage in self.TRANSFER_STORAGE_LIST:
            try:
                ...
                if (consul_config.get("cluster_type") in IGNORED_STORAGE_CLUSTER_TYPES) or (
                    ...
                    and table_id in settings.metadata.skip_influxdb_table_id_list
                ):
                    continue
    ...
    def to_json(self, is_consul_config=False, with_rt_info=True) -> dict[str, Any]:
        """返回当前data_id的配置字符串"""
    ...
    def register_to_bkbase(self, bk_biz_id: int, namespace: str = "bkmonitor", bkbase_data_name: str = ""):
        ...
        from bk_monitor_base.metadata.models.data_link import DataIdConfig, utils
    ...
    def apply_for_data_id_from_bkdata(
        ...
        """
        # 下发配置
        from bk_monitor_base.metadata.models.data_link.constants import DataLinkResourceStatus
        from bk_monitor_base.metadata.models.data_link.service import apply_data_id_v2, get_data_id_v2
    ...
    def create_data_source(
        ...
        is_refresh_config: bool = True,
        custom_label: str | None = None,
        option=None,
        transfer_cluster_id=None,
        source_system: str = settings.blueking.app_code,
        space_type_id=None,
        space_id=None,
        ...
        # 判断两个使用到的标签是否存在
        logger.info(
            "create_data_source: try to create datasource with params,data_name->[%s],etl_config->[%s], operator->[%s],source_label->[%s],type_label->[%s],bk_tenant_id->[%s]",
            data_name,
            etl_config,
            operator,
            source_label,
        ...
        if not Label.exists_label(label_id=source_label, label_type=Label.LABEL_TYPE_SOURCE) or not Label.exists_label(
            ...
            logger.error(
                f"user->[{operator}] try to create datasource but use type_label->[{type_label}] or source_type->[{source_label}] which is not exists."
            )
        ...
        if bk_data_id is None and settings.metadata.is_assign_dataid_by_gse:
            ...
            from bk_monitor_base.metadata.models.space.constants import ENABLE_V4_DATALINK_ETL_CONFIGS

            # 开启V4链路后，特定etl_config的data_id均从计算平台获取
            enabled_custom_event_v4 = (
                etl_config == EtlConfigs.BK_STANDARD_V2_EVENT.value
                and settings.metadata.enable_v4_event_group_data_link
            )
            if (
                settings.metadata.enable_v2_vm_data_link and etl_config in ENABLE_V4_DATALINK_ETL_CONFIGS
        ...
        with atomic(config.DATABASE_CONNECTION_NAME):
            if transfer_cluster_id is None:
                transfer_cluster_id = settings.metadata.default_transfer_cluster_id
            ...
            if not settings.metadata.is_assign_dataid_by_gse:
                ...
                if data_source.bk_data_id > config.MAX_DATA_ID:
                    logger.info(
                        f"new data_id->[{data_source.bk_data_id}] is lager than max data id->[{config.MAX_DATA_ID}], nothing will create.",
                    )
    ...
    def update_config(
        ...
        :param is_platform_data_id: 是否平台级数据源
        :param space_type_id: 空间类型
        :param space_uid: 空间 uid
        :param created_from: 数据源 ID 来源
        :param authorized_spaces: 授权的使用的空间
        ...
        if data_name is not None and self.data_name != data_name:
            if self.__class__.objects.filter(data_name=data_name).exists():
                logger.error(
                    f"user->[{operator}] try to update data_id->{self.bk_data_id}] data_name->[{data_name}] but data name is already exists, nothing will do"
                )
    ...
    def add_built_in_channel_id_to_gse(self):
        ...
        result = api.gse.add_route(
            bk_tenant_id=self.bk_tenant_id or DEFAULT_TENANT_ID,
    ...
    def register_to_gse(self) -> bool:
        ...
        try:
            result = api.gse.query_route(
                ...
                operation={"operator_name": DEFAULT_USERNAME},
            )
            return True
        except BkApiError as e:
    ...
    def refresh_gse_config_to_gse(self):
        ...
        try:
            result = api.gse.query_route(bk_tenant_id=self.bk_tenant_id or DEFAULT_TENANT_ID, **params)
            if not result:
        ...
        api.gse.update_route(bk_tenant_id=self.bk_tenant_id or DEFAULT_TENANT_ID, **params)
