...
from bk_monitor_base.metadata.models import common
from bk_monitor_base.metadata.models.custom_report import EventGroup, TimeSeriesGroup
...
class BCSClusterInfo(models.Model):
    ...
    def k8s_client_config(self) -> k8s_client.Configuration:
        """返回一个可用的k8s集群配置信息"""
        host = (
            f"{settings.metadata.bcs_api_gateway_schema}://"
    ...
    def bk_env_label(self) -> str:
        ...
        return self.bk_env or settings.metadata.bcs_cluster_bk_env_label
    ...
    def register_cluster(
        ...
        creator: str,
        bk_tenant_id: str,
        domain_name: str = settings.metadata.bcs_api_gateway_host,
        port: int = settings.metadata.bcs_api_gateway_port,
        api_key_type: str = "authorization",
        api_key_prefix: str = "Bearer",
        is_skip_ssl_verify: bool = True,
        transfer_cluster_id: str | None = None,
        bk_env: str = settings.metadata.bcs_cluster_bk_env_label,
        ...
        if cls.objects.filter(cluster_id=cluster_id).exists():
            logger.error(
                "failed to register cluster_id->[%s] under project_id->[%s] for cluster is already register, nothing will do any more"
            )
            raise ValueError(_("集群已经接入，请确认后重试"))

        # 直接基于环境变量进行bcs纳管k8s信息的填充
        api_key_content = settings.metadata.bcs_api_gateway_token
        ...
        cluster = cls.objects.create(
            ...
            bk_env=bk_env or settings.metadata.bcs_cluster_bk_env_label,
        )
        logger.info(
        ...
        if transfer_cluster_id is None:
            transfer_cluster_id = settings.metadata.default_transfer_cluster_id_for_k8s
        ...
        for usage, register_info in cluster.DATASOURCE_REGISTER_INFO.items():
            ...
            data_source = cluster.create_datasource(
                ...
                operator=creator,
                mq_cluster_id=settings.metadata.bcs_kafka_storage_cluster_id,
            ...
            if register_info["usage"] == "metric":
                ...
                default_storage_config = {
                    "proxy_cluster_name": settings.metadata.influxdb_default_proxy_cluster_name_for_k8s
                }
                additional_options = {
                    ResultTableOption.OPTION_CUSTOM_REPORT_DIMENSION_VALUES: cls.DEFAULT_SERVICE_MONITOR_DIMENSION_TERM
                }
            else:
                default_storage_config = {"cluster_id": settings.metadata.bcs_custom_event_storage_cluster_id}
    ...
    def compose_dataid_resource_label(self, labels: dict[str, Any]) -> dict[str, Any]:
        """组装下发的配置资源的标签"""
        if self.bk_env_label:
    ...
    def _ensure_relation_dataid_resource(self, resource_items: dict[str, Any] | None = None):
    ...
    def refresh_cluster_bcs_info(
        ...
        domain_name: str = settings.metadata.bcs_api_gateway_host,
        port: int = settings.metadata.bcs_api_gateway_port,
        api_key_content: str = settings.metadata.bcs_api_gateway_token,
    ...
    def make_config(self, item, usage, is_fed_cluster: bool = False) -> dict[str, Any]:
        # 获取全局的replace配置
    ...
    def create_datasource(
        ...
        try:
            datasource = DataSource.create_data_source(
                ...
                transfer_cluster_id=transfer_cluster_id,
                source_system=get_config().blueking.app_code,
                bcs_cluster_id=self.cluster_id,
    ...
    def to_json_for_user(self) -> dict[str, Any]:
        """
        返回必要信息
        """
        return {
...
class BcsFederalClusterInfo(common.BaseModelWithTime):
    ...
    fed_namespaces = models.JSONField("命名空间列表", default=list)
