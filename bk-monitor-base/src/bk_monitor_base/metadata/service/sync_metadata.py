...
def sync_kafka_metadata(bk_tenant_id: str, kafka_info: dict[str, Any], ds: models.DataSource, bk_data_id: int):
    ...
    if not kafka_cluster:
        logger.info(
            "sync_kafka_metadata: data different,kafka_cluster does not exist,try to create,data->[%s], bk_data_id->[%s]",
            kafka_info,
            bk_data_id,
        )
        ...
        if settings.metadata.enable_sync_bkbase_metadata_to_db:
            logger.info(
    ...
    try:
        ...
        if (kafka_topic_ins.topic != kafka_info["topic"] and kafka_info["topic"]) or (
            ...
            logger.info(
                "sync_kafka_metadata: data different,kafka_topic_info is different from old,try to update,bk_data_id->[%s],old_topic->[%s],new_topic->[%s],old_partitions->[%s],new_partitions->[%s]",
                bk_data_id,
