...
class Command(BaseCommand):
    ...
    def _list_es_router(self) -> Queue:
        # 拉取总数量
        try:
            data = api.log_search.list_es_router(bk_tenant_id=DEFAULT_TENANT_ID)
    ...
    def _request_es_router(self, page: int, data_queue: Queue):
        data = api.log_search.list_es_router(bk_tenant_id=DEFAULT_TENANT_ID, page=page, pagesize=self.PAGE_SIZE)
