...
from bk_monitor_base.metadata.config import BASE_DIR
from bk_monitor_base.metadata.models.space.constants import SYSTEM_USERNAME, EtlConfigs, SpaceTypes
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 4. 创建结果表
        # 获取初始化数据
        bkci_data_path = os.path.join(BASE_DIR, "metadata/data/bkci_data.json")
    ...
    def _get_or_create_result_table(self, bk_data_id: int):
        ...
        for table in existed_table_qs:
            ...
            try:
                ...
                self.stderr.write(f"update result table failed, field_list: {json.dumps(field_list)}, err: {e}")
            else:
