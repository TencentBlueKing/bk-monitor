...
from bk_monitor_base.metadata.models.bcs.cluster import BCSClusterInfo
from bk_monitor_base.metadata.models.bcs.resource import PodMonitorInfo, ServiceMonitorInfo
...
from bk_monitor_base.metadata.models.custom_report.subscription_config import CustomReportSubscription
from bk_monitor_base.metadata.models.data_link.data_link import DataLink
from bk_monitor_base.metadata.models.data_link.utils import compose_bkdata_data_id_name, compose_bkdata_table_id
from bk_monitor_base.metadata.models.data_source import DataSource, DataSourceOption
from bk_monitor_base.metadata.models.influxdb_cluster import InfluxDBClusterInfo, InfluxDBHostInfo
...
from bk_monitor_base.metadata.models.space.space import Space, SpaceDataSource
from bk_monitor_base.metadata.models.storage import (
    ClusterInfo,
    DorisStorage,
    ESStorage,
...
from bk_monitor_base.metadata.utils import consul_tools, hash_util


def recode_final_result(fun: Callable[..., Any]) -> Callable[..., Any]:
    """记录最终结果的装饰器"""
    status_priority = {
        Status.UNKNOWN: 0,
        Status.SUCCESS: 1,
    ...
    def inner(self: "Command", *args, **kwargs) -> Any:
...
class Command(BaseCommand):
    ...
    def data_sources(self) -> dict[str, DataSource]:
        ...
        data_ids = [_id for _id in data_ids if _id != 0]
    ...
    def check_database_record(self, cluster_id: str) -> dict:
        ...
        def format_output(details: dict[str, Any]) -> list[str]:
            """格式化数据库检查输出"""
            lines: list[str] = []
            if details:
    ...
    def check_bcs_api_token(self, cluster_info: BCSClusterInfo) -> dict:
        ...
        try:
            ...
            # 检查API密钥是否与当前配置一致
            if cluster_info.api_key_content != settings.metadata.bcs_api_gateway_token:
                message = f"[BCSClusterInfo] [cluster_id={cluster_info.cluster_id}] "
            ...
            result["details"] = {
                ...
                "api_key_match": cluster_info.api_key_content == settings.metadata.bcs_api_gateway_token,
    ...
    def check_custom_report_subscription(self, cluster_info: BCSClusterInfo) -> dict:
        ...
        try:
            ...
            for data_id, _datasource in self.data_sources.items():
                try:
                    # 查询此数据源的订阅配置
    ...
    def check_influxdb_storage_config(self, cluster_info: BCSClusterInfo) -> dict:
        ...
        try:
            ...
            for data_id, _data_source in self.data_sources.items():
                try:
                    ds_rt = DataSourceResultTable.objects.filter(
                        bk_data_id=data_id, bk_tenant_id=self.bk_tenant_id
                    ).first()
            ...
            for data_id, data_source in self.data_sources.items():
                ...
                try:
                    ...
                    if (
                        ...
                        and not settings.metadata.enable_v2_vm_data_link
                    ):
                        need_continue = True

                    # 没开启INFLUXDB存储，需要进行vm数据链路检查
                    if not settings.metadata.enable_influxdb_storage:
                        need_continue = False
                    ...
                    if vm_record and need_continue:
                        result["warnings"].append(
                            f"[ENABLE_V2_VM_DATA_LINK={settings.metadata.enable_v2_vm_data_link}]"
                            f"[ENABLE_INFLUXDB_STORAGE={settings.metadata.enable_influxdb_storage}]"
                    ...
                    data_link = DataLink.objects.filter(
                        ...
                        namespace=settings.metadata.default_vm_data_link_namespace,
            ...
            for data_id, data_source in self.data_sources.items():
                ...
                try:
                    params = {
                        ...
                        "operation": {"operator_name": get_config().blueking.common_username},
                    }
