...
class Command(BaseCommand):
    ...
    def _bulk_create_proxy_storage(self, proxy_cluster_list: QuerySet, proxy_storage_map: dict) -> bool:
        ...
        influxdb_proxy_storage = models.InfluxDBProxyStorage.objects.filter(
            Q(instance_cluster_name=settings.metadata.influxdb_default_proxy_cluster_name)
            | Q(instance_cluster_name=settings.metadata.influxdb_default_proxy_cluster_name_for_k8s),
        ...
        if not influxdb_proxy_storage:
            self.stderr.write(
                ...
                f"instance_cluster_name: {settings.metadata.influxdb_default_proxy_cluster_name}"
            )
    ...
    def _update_router_field(self):
        ...
        for ps, storage_id in proxy_storage_map.items():
            ...
            if not router.exists():
                ...
                continue
            router.update(influxdb_proxy_storage_id=storage_id)
