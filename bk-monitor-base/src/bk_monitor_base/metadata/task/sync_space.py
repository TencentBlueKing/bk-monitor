...
from bk_monitor_base.infras.third_party_api.cmdb.entity import Biz
...
from bk_monitor_base.metadata.models.constants import BULK_CREATE_BATCH_SIZE, BULK_UPDATE_BATCH_SIZE
from bk_monitor_base.metadata.models.space import Space, SpaceDataSource, SpaceResource
...
from bk_monitor_base.metadata.task.tasks import check_bkcc_space_builtin_datalink
from bk_monitor_base.metadata.task.utils import bulk_handle
from bk_monitor_base.metadata.utils.api_request import bulk_request
...
def sync_bkcc_space(
    bk_tenant_id: str | None = None, allow_deleted: bool = False, create_builtin_data_link_delay: bool = True
):
    ...
    for tenant_id in bk_tenant_ids:
        _, temp = api.cmdb.search_business(bk_tenant_id=tenant_id)
        for b in temp:
            b.bk_tenant_id = tenant_id
...
def sync_archived_bkcc_space(bk_tenant_id: str | None = None):
    ...
    bkcc_type_id = SpaceTypes.BKCC.value
    # 获取已归档的业务
    archived_biz_list: list[Biz] = []

    if bk_tenant_id:
        bk_tenant_ids: list[str] = [bk_tenant_id]
    else:
        bk_tenant_ids: list[str] = [tenant["id"] for tenant in list_tenant()]

    for tenant_id in bk_tenant_ids:
        _, biz_list = api.cmdb.search_business(
            bk_tenant_id=tenant_id,
            condition={"bk_data_status": "disabled"},
        )
        for biz in biz_list:
            biz.bk_tenant_id = tenant_id
        archived_biz_list.extend(biz_list)
...
def refresh_bkcc_space_name():
    ...
    biz_map: dict[str, Biz] = {}
    for tenant in api.user.list_tenant():
        _count, biz_list = api.cmdb.search_business(bk_tenant_id=tenant["id"])
        biz_map.update({str(b.bk_biz_id): b for b in biz_list})
...
def push_and_publish_space_router(
    ...
    """推送数据和通知"""
    from bk_monitor_base.metadata.models.space.constants import SPACE_TO_RESULT_TABLE_CHANNEL
    from bk_monitor_base.metadata.models.space.ds_rt import get_space_table_id_data_id
    from bk_monitor_base.metadata.models.space.space_table_id_redis import SpaceTableIDRedis

    # 过滤数据
    spaces = models.Space.objects.only("space_type_id", "space_id", "bk_tenant_id")
    if space_type:
    ...
    if settings.blueking.enable_multi_tenancy:  # 若开启多租户模式，则以空间粒度推送路由
        for space in spaces:
            tid_ds = get_space_table_id_data_id(space.space_type_id, space.space_id)
...
def authorize_paas_space_cluster_data_source(space_cluster: dict):
    ...
    paas_data_id_list = settings.metadata.bkpaas_authorized_data_id_list
    # 进行数据匹配
    for space_id, data_ids in space_data_id_map.items():
        # 匹配到需要增加的数据源
...
def refresh_bksaas_space_resouce():
    ...
    params_list: list[dict[str, Any]] = [
        {"bk_tenant_id": space.bk_tenant_id, "app_code": space.space_id} for space in spaces
    ]
    # 批量获取蓝鲸应用使用的集群资源
    space_id_count = len(params_list)
    # 分组
    params_segment_list = [params_list[i : i + MAX_PAGE_NUM] for i in range(0, space_id_count, MAX_PAGE_NUM)]

    # 然后根据分组进行请求
    space_cluster_namespaces, space_cluster = {}, {}
    for params_list in params_segment_list:
        cluster_ns = bulk_request(api.bk_paas.get_app_cluster_namespace, params_list)
        # 组装数据
        for space_dict, val in zip(params_list, cluster_ns, strict=False):
