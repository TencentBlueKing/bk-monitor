...
from bk_monitor_base.metadata.config import PERIODIC_TASK_DEFAULT_TTL, settings
...
from bk_monitor_base.metadata.task.tasks import bulk_create_fed_data_link
from bk_monitor_base.metadata.utils.api_request import bulk_request
from bk_monitor_base.metadata.utils.bcs import change_cluster_router, get_bcs_dataids
...
@app.task(ignore_result=True, queue=settings.metadata.celery_queue or "celery_cron")
def refresh_dataid_resource(cluster_id: str, data_id: int):
...
def discover_bcs_clusters():
    ...
    for tenant in list_tenant():
        ...
        try:
            raw_bcs_clusters = api.bcs_cluster_manager.fetch_k8s_cluster_list(bk_tenant_id=bk_tenant_id)
            bcs_clusters: list[dict[str, Any]] = []
            cluster_id_set: set[str] = set()
            for bcs_cluster in raw_bcs_clusters:
        ...
        for bcs_cluster in bcs_clusters:
            ...
            if cluster:
                ...
                # 如果 BCS Token 变了需要刷新
                if cluster.api_key_content != settings.metadata.bcs_api_gateway_token:
                    cluster.api_key_content = settings.metadata.bcs_api_gateway_token
    ...
    if cluster_list:
        logger.info(
            ...
            settings.metadata.always_running_fake_bcs_cluster_id_list,
        )
        cluster_list.extend(settings.metadata.always_running_fake_bcs_cluster_id_list)
...
def update_bcs_cluster_cloud_id_config(bk_biz_id: int | None = None, cluster_id: str | None = None):
    """补齐云区域ID ."""
    # 获得缺失云区域的集群配置
    filter_kwargs = {}
    if bk_biz_id:
    ...
    for start in range(0, len(clusters), BCS_SYNC_SYNC_CONCURRENCY):
        ...
        if not cmdb_params:
            continue
        try:
            host_infos = bulk_request(api.cmdb.get_host_by_ip, cmdb_params)
        except Exception as exc_info:  # noqa
            logger.exception(exc_info)
            continue
        bk_cloud_map = {}
...
def sync_federation_clusters(fed_clusters: dict[str, Any]):
    """
    ...
    need_process_clusters: list[str] = []  # 记录需要创建联邦汇聚链路的集群列表，统一进行异步操作
    try:
        # 获取传入数据中的所有联邦集群 ID
        ...
        for fed_cluster_id, fed_cluster_data in fed_clusters.items():
            ...
            for sub_cluster_id, namespaces in sub_clusters.items():
                ...
                logger.info(
                    "sync_federation_clusters:Updated federation cluster info for sub-cluster->[%s] in fed->[%s] successfully，will create fed data_link later",
                    sub_cluster_id,
                    fed_cluster_id,
                )
        ...
        bulk_create_fed_data_link.delay(list(set(need_process_clusters)))
