...
from bk_monitor_base.metadata.models import BkBaseResultTable, ClusterInfo, DataSource
...
from bk_monitor_base.metadata.models.data_link.data_link import DataLink
from bk_monitor_base.metadata.models.data_link.service import get_data_link_component_status
from bk_monitor_base.metadata.models.space.constants import EtlConfigs, SpaceTypes
from bk_monitor_base.metadata.models.space.space import Space
from bk_monitor_base.metadata.models.space.space_table_id_redis import SpaceTableIDRedis
from bk_monitor_base.metadata.models.storage import ESStorage
from bk_monitor_base.metadata.models.vm.record import AccessVMRecord
from bk_monitor_base.metadata.models.vm.utils import (
    create_fed_bkbase_data_link,
    get_vm_cluster_id_name,
    report_metadata_data_link_status_info,
)
from bk_monitor_base.metadata.service.sync_metadata import sync_kafka_metadata, sync_vm_metadata
from bk_monitor_base.metadata.task.utils import bulk_handle
from bk_monitor_base.metadata.utils import consul_tools
from bk_monitor_base.metadata.utils.redis_tools import RedisTools, bkbase_redis_client
from bk_monitor_base.metadata.utils.tenant import get_tenant_default_biz_id
...
def refresh_custom_report_config(bk_biz_id: int | None = None):
    from bk_monitor_base.metadata.task.custom_report import refresh_custom_report_2_node_man
...
def refresh_custom_log_report_config(log_group_id: int | None = None):
    from bk_monitor_base.metadata.task.custom_report import refresh_custom_log_config
...
def refresh_entity_definition_to_redis():
    from bk_monitor_base.metadata.task.entity_relation import refresh_entity_definition_to_redis as _refresh
...
@app.task(ignore_result=True, queue=settings.metadata.celery_queue or "celery_metadata_task_worker")
def create_full_cmdb_level_data_flow(table_id: str, bk_tenant_id: str = DEFAULT_TENANT_ID):
    try:
...
def manage_es_storage(storage_record_ids, cluster_id: int = None):
    ...
    for es_storage in es_storages:
        ...
        try:
            ...
            time.sleep(settings.metadata.es_index_rotation_sleep_interval_seconds)  # 等待一段时间，降低负载
            logger.info(
...
def _clean_disable_es_storage(es_storage):
    ...
    cost_time = time.time() - start_time
    logger.info(
        "_clean_disable_es_storage: end to _clean_disable_es_storage,table_id->[%s], cost->[%s]",
        es_storage.table_id,
        cost_time,
    )
...
def _manage_es_storage(es_storage):
    ...
    cost_time = time.time() - start_time
    logger.info("manage_es_storage:table_id->[%s] end to create index, cost->[%s]", es_storage.table_id, cost_time)
...
def push_and_publish_space_router(
    ...
    from bk_monitor_base.metadata.models.space.ds_rt import get_space_table_id_data_id
    from bk_monitor_base.metadata.models.space.space_table_id_redis import SpaceTableIDRedis
...
def _access_bkdata_vm(
    ...
    """
    from bk_monitor_base.metadata.models.vm.utils import access_bkdata, access_v2_bkdata_vm

    # NOTE：只有当allow_access_v2_data_link为True，即单指标单表时序指标数据时，才允许接入V4链路
    if settings.metadata.enable_v2_vm_data_link and allow_access_v2_data_link:
...
def push_space_to_redis(space_type: str, space_id: str):
    ...
    try:
        from bk_monitor_base.metadata.models.space.constants import SPACE_REDIS_PREFIX_KEY
...
def _check_and_delete_ds_consul_config(data_source: DataSource):
    ...
    logger.info(
        "_check_and_delete_ds_consul_config:data_source->[%s],consul_config deleted, cost->[%s]",
...
@app.task(ignore_result=True, queue=settings.metadata.celery_queue or "celery_metadata_task_worker")
def bulk_create_fed_data_link(sub_clusters: list[str]):
...
@app.task(ignore_result=True, queue=settings.metadata.celery_queue or "celery_metadata_task_worker")
def sync_bkbase_v4_metadata(key: str, skip_types: list[str] | None = None):
    """
...
def process_gse_slot_message(message_id: str, bk_agent_id: str, content: str, received_at: str):
    ...
    if content_data.get("type") == "fetch/host/dataid":
        ...
        if settings.blueking.space_builtin_data_link_mode == "tenant":
            try:
            ...
            _count, all_hosts = api.cmdb.list_all_hosts(bk_tenant_id=bk_tenant_id)
            host = None
            for h in all_hosts:
                if getattr(h, "bk_agent_id", None) == bk_agent_id:
                    host = h
                    break
            if not host:
                logger.warning(
...
def check_bkcc_space_builtin_datalink(biz_list: list[tuple[str, int]]):
    ...
    if (
        not (settings.metadata.enable_v2_vm_data_link and settings.metadata.enable_space_builtin_data_link)
        or not biz_list
    ):
        return
...
def create_basereport_datalink_for_bkcc(bk_tenant_id: str, bk_biz_id: int, storage_cluster_name: str | None = None):
    ...
    # TASK2 -- 计算平台V4链路部分
    logger.info(
        "create_basereport_datalink_for_bkcc: now try to create bkbase part,bk_biz_id->[%s],bk_tenant_id->[%s],bk_data_id->[%s]",
        bk_biz_id,
        bk_tenant_id,
        data_source.bk_data_id,
    )
    ...
    # 1. 创建DataLink 链路管理实例
    logger.info(
        "create_basereport_datalink_for_bkcc: now try to create data link instance,bk_biz_id->[%s], data_link_name->[%s]",
        bk_biz_id,
        data_name,
    )
            ...
            if not es_storage:
                es_storage = models.ESStorage.objects.create(
                    ...
                    index_settings=json.dumps(
                        {
                            "number_of_shards": settings.metadata.system_event_default_es_index_shards,
                            "number_of_replicas": settings.metadata.system_event_default_es_index_replicas,
                        }
                    ),
...
def create_single_tenant_system_datalink(
    ...
    if data_bk_biz_id is None:
        bk_biz_id = settings.metadata.default_bkdata_biz_id
    else:
        bk_biz_id = data_bk_biz_id

    datasource = DataSource.objects.get(bk_data_id=config.SNAPSHOT_DATAID)
...
def create_single_tenant_system_proc_datalink(
    ...
    data_ids = [config.PROCESS_PERF_DATAID, config.PROCESS_PORT_DATAID]
    data_id_to_table_id = {
        config.PROCESS_PERF_DATAID: "system.proc",
        config.PROCESS_PORT_DATAID: "system.proc_port",
    }
    data_id_to_bkbase_data_name = {
        config.PROCESS_PERF_DATAID: "system_proc_perf",
        config.PROCESS_PORT_DATAID: "system_proc_port",
    }
    data_id_to_data_link_strategy = {
        config.PROCESS_PERF_DATAID: DataLink.SYSTEM_PROC_PERF,
        config.PROCESS_PORT_DATAID: DataLink.SYSTEM_PROC_PORT,
    }
