...
from bk_monitor_base.metadata.task.bkbase import sync_bkbase_cluster_info
from bk_monitor_base.metadata.task.constants import BKBASE_V4_KIND_STORAGE_CONFIGS
from bk_monitor_base.metadata.task.sync_space import sync_bkcc_space
...
def _init_es_cluster(bk_tenant_id: str) -> bool:
    """
    初始化ES集群
    """
...
def init_tenant(bk_tenant_id: str):
    ...
    # 标记已初始化的租户
    if bk_tenant_id not in settings.blueking.initialized_tenant_list:
        logger.info(f"tenant({bk_tenant_id}) is initialized.")
        settings.blueking.initialized_tenant_list = settings.blueking.initialized_tenant_list + [bk_tenant_id]
    else:
