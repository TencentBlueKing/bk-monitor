...
from bk_monitor_base.metadata.models.constants import NEED_REFRESH_DATA_IDS, DataIdCreatedFromSystem, EsSourceType
...
def refresh_es_storage():
    ...
    es_blacklist = settings.metadata.es_cluster_blacklist
...
def manage_disable_es_storage():
    ...
    for cluster in es_storages_by_cluster:
        try:
            ...
            # 默认使用新方式轮转
            clean_disable_es_storage.delay(list(cluster_storages), cluster_id)
        except Exception as e:  # pylint: disable=broad-except
            logger.error(
                "manage_disable_es_storage:refresh cluster_id->[%s] failed for->[%s]", cluster["storage_cluster_id"], e
            )
            continue
...
def check_and_delete_ds_consul_config():
    ...
    bulk_check_and_delete_ds_consul_config.delay(list(data_sources))
...
def clean_datasource_from_consul():
    ...
    transfer_id_and_data_ids: dict[str, Any] = {}
    for d in data_info:
    ...
    transfer_cluster_ids: set[str] = set()
    for d in transfer_consul_data[1]:
        try:
            transfer_cluster_id: str = d["Key"].split(transfer_key_tmpl)[-1].split("/")[0]
        except Exception as e:
            logger.error("parse consul data id error, data: %s error: %s", d, e)
            continue
        transfer_cluster_ids.add(transfer_cluster_id)
    diff_transfers: set[str] = transfer_cluster_ids - set(transfer_id_and_data_ids.keys())
    ...
    logger.info("deleted transfer cluster: %s", json.dumps(list(diff_transfers)))
    ...
    for transfer_id, data_ids in transfer_id_and_data_ids.items():
        ...
        diff_data_ids: set[str] = consul_data_ids - data_ids
        if not diff_data_ids:
            continue

        logger.info("need delete data id: %s", json.dumps(list(diff_data_ids)))
