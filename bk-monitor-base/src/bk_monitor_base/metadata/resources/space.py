...
from bk_monitor_base.metadata.models.space import constants, utils
...
from bk_monitor_base.metadata.service.space_redis import (
    get_kihan_prom_field_list,
    get_space_config_from_redis,
)
from bk_monitor_base.metadata.utils.redis_tools import RedisTools
from bk_monitor_base.metadata.utils.request import get_request, get_request_tenant_id
from bk_monitor_base.metadata.utils.user import get_local_username, get_request_username
...
class UpdateSpaceResource(Resource):
    class RequestSerializer(SpaceSerializer):
        ...
        def validate(self, data: OrderedDict) -> OrderedDict:
            data = super().validate(data)
            # 校验空间存在
            space.Space.objects.get(space_type_id=data["space_type_id"], space_id=data["space_id"])
            # 校验名称唯一
...
class MergeSpaceResource(Resource):
    class RequestSerializer(serializers.Serializer):
        ...
        def validate(self, data: OrderedDict) -> OrderedDict:
            ...
            space.Space.objects.get(space_type_id=dst_space_type_id, space_id=dst_space_id)
