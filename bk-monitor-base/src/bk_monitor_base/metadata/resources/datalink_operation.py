...
from bk_monitor_base.infras.third_party_api.cmsi.api import Attachment
...
from bk_monitor_base.metadata.models.space.utils import get_related_spaces
from bk_monitor_base.metadata.resources.base import Resource
from bk_monitor_base.metadata.utils.basic import get_biz_id_by_space_uid
from bk_monitor_base.metadata.utils.serializers import TenantIdField

logger = logging.getLogger("metadata")


def extract_cluster_id(data_name: str) -> str | None:
    # 定义匹配集群ID的正则表达式
    pattern = r"(BCS-K8S-\d+)"
    # 使用re.search查找匹配的内容
...
class SpaceDataLinkMetaReport(Resource):
    ...
    def perform_request(self, validated_request_data: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(
            "SpaceDataLinkMetaReport: try to get space info file, space_type->[%s], space_id->[%s], with_related_spaces->[%s],send_mail->[%s]",
            space_type,
            space_id,
            with_related_spaces,
            send_mail,
        ...
        if space_type != "bkcc":
            space_record: models.Space = models.Space.objects.get(space_type_id=space_type, space_id=space_id)
            bk_biz_id: int = -space_record.pk
        ...
        for data_id in data_ids:
            ...
            try:
                ...
                if cluster_id:
                    ...
                    if cluster_bk_biz_id != expected_bk_biz_id:  # 若集群归属业务不符合预期，跳过本次循环
                        logger.info(
                            "SpaceDataLinkMetaReport: cluster_id->[%s], data_id->[%s], not independent cluster, cluster belongs to->[%s]",
                            cluster_id,
                            ds.bk_data_id,
                            cluster_bk_biz_id,
                        )
            ...
            table_ids: list[str] = table_ids + temp_table_ids
        ...
        if related_spaces:
            for related_space in related_spaces:
                space_record: models.Space = models.Space.objects.get(
                    bk_tenant_id=bk_tenant_id, space_type_id="bkci", space_id=related_space
                )
                related_bk_biz_id: int = -space_record.pk
        ...
        space_name: str = models.Space.objects.get(
