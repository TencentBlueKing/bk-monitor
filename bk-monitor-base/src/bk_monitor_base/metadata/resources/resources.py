...
from bk_monitor_base.metadata.config import (
    ES_ROUTE_ALLOW_URL,
...
from bk_monitor_base.metadata.constants.data_source import DATA_LINK_V4_VERSION_NAME
from bk_monitor_base.metadata.models.bcs import (
    BCSClusterInfo,
    LogCollectorInfo,
    PodMonitorInfo,
    ServiceMonitorInfo,
)
from bk_monitor_base.metadata.models.constants import (
    DT_TIME_STAMP_NANO,
    EPOCH_MILLIS_FORMAT,
    NANO_FORMAT,
...
from bk_monitor_base.metadata.models.data_link.constants import BKBASE_NAMESPACE_BK_LOG
from bk_monitor_base.metadata.models.data_link.data_link_configs import DataIdConfig
from bk_monitor_base.metadata.models.data_link.utils import (
    compose_bkdata_data_id_name,
    get_bkbase_raw_data_name_for_v3_datalink,
    get_data_source_related_info,
)
from bk_monitor_base.metadata.models.result_table import ResultTableOption
from bk_monitor_base.metadata.models.space.constants import SPACE_UID_HYPHEN, EtlConfigs, SpaceTypes
from bk_monitor_base.metadata.models.space.space_table_id_redis import SpaceTableIDRedis
from bk_monitor_base.metadata.resources.base import Resource
from bk_monitor_base.metadata.resources.cluster import (
    CreateClusterInfoResource,
    DeleteClusterInfoResource,
    ModifyClusterInfoResource,
    QueryClusterInfoResource,
)
from bk_monitor_base.metadata.service.data_source import (
    modify_data_id_source,
    stop_or_enable_datasource,
)
from bk_monitor_base.metadata.service.storage_details import ResultTableAndDataSource
from bk_monitor_base.metadata.task.bcs import refresh_dataid_resource
from bk_monitor_base.metadata.utils import consul
from bk_monitor_base.metadata.utils.bcs import get_bcs_dataids
from bk_monitor_base.metadata.utils.bkbase import sync_bkbase_result_table_meta
from bk_monitor_base.metadata.utils.data_link import get_record_rule_metrics_by_biz_id
from bk_monitor_base.metadata.utils.es_tools import get_client
from bk_monitor_base.metadata.utils.k8s_metric import get_built_in_k8s_events, get_built_in_k8s_metrics
from bk_monitor_base.metadata.utils.request import get_app_code_by_request, get_request, get_request_tenant_id
from bk_monitor_base.metadata.utils.serializers import TenantIdField
...
class GetOrCreateAgentEventDataIdResource(Resource):
    ...
    def perform_request(self, validated_request_data: dict[str, Any]):
        ...
        logger.info(
            "GetOrCreateAgentEventDataIdResource: try to create agent event data id for bk_biz_id->[%s],use data_name->[%s]",
            bk_biz_id,
            data_name,
        )
...
class CreateResultTableResource(Resource):
    ...
    @override
    def perform_request(self, validated_request_data: dict[str, Any]):
        query_alias_settings = validated_request_data.pop("query_alias_settings", [])
        ...
        if query_alias_settings:
            try:
                logger.info(
                    "CreateResultTableResource: try to manage alias_settings,table_id->[%s],query_alias_settings->[%s],bk_tenant_id->[%s]",
                    table_id,
                ...
                logger.warning(
                    "CreateResultTableResource: manage alias_settings failed,table_id->[%s],query_alias_settings->[%s],error->[%s]",
                    table_id,
                    query_alias_settings,
                    e,
                )
        try:
            new_result_table = models.ResultTable.create_result_table(**validated_request_data)
            ...
            raise e
        return new_result_table.table_id


class PageSerializer(serializers.Serializer):
    page: serializers.IntegerField = serializers.IntegerField(default=1, required=False, label="页数", min_value=1)
    page_size: serializers.IntegerField = serializers.IntegerField(default=0, required=False, label="页长")
...
class ListResultTableResource(Resource):
    ...
    @override
    def perform_request(self, validated_request_data: dict[str, Any]) -> list[dict[str, Any]] | dict[str, Any]:
        bk_tenant_id = validated_request_data["bk_tenant_id"]
        ...
        # 判断是否有结果表类型的过滤
        datasource_type = validated_request_data["datasource_type"]
        if datasource_type is not None:
            result_table_queryset = result_table_queryset.filter(table_id__startswith=f"{datasource_type}.")

        # 判断是否有全业务和单业务的过滤需求
        bk_biz_id = []
        if validated_request_data["is_public_include"] is not None and validated_request_data["is_public_include"]:
            bk_biz_id.append(0)

        if validated_request_data["bk_biz_id"] is not None:
            bk_biz_id.append(validated_request_data["bk_biz_id"])
        ...
        if len(bk_biz_id) != 0:
            ...
            for biz_id in bk_biz_id:
                ...
                try:
                    ...
                    logger.error(
                        "ListResultTableResource: get_record_rule_metrics_by_biz_id failed, bk_biz_id->[%s], error->[%s]",
                        bk_biz_id,
                        e,
                    )
                    precal_metrics = []
        ...
        # 判断是否需要带上非用户字段的内容
        if validated_request_data["is_config_by_user"]:
        ...
        if validated_request_data["page_size"] > 0:
            ...
            result_list = models.ResultTable.batch_to_json(
                ...
                with_option=validated_request_data["with_option"],
            )
...
class ModifyResultTableResource(Resource):
    ...
    def _handle_query_alias_settings(
        ...
        try:
            ...
            logger.warning(
                "ModifyResultTableResource: manage alias_settings failed,table_id->[%s],query_alias_settings->[%s],error->[%s]",
                table_id,
                query_alias_settings,
                e,
            )
...
class AccessBkDataByResultTableResource(Resource):
    ...
    @override
    def perform_request(self, validated_request_data: dict[str, Any]):
        if not settings.metadata.is_access_bk_data:
            return
...
class CreateDownSampleDataFlowResource(Resource):
    ...
    def perform_request(self, validated_request_data: dict[str, Any]):
        ...
        from bk_monitor_base.metadata.task import tasks
...
class QueryDataSourceResource(Resource):
    ...
    @override
    def perform_request(self, validated_request_data: dict[str, Any]) -> dict[str, Any]:
        if validated_request_data["bk_data_id"] is not None:  # 指定bk_data_id时，无需添加租户过滤条件
            data_source = models.DataSource.objects.get(bk_data_id=validated_request_data["bk_data_id"])
        elif validated_request_data["data_name"] is not None:
            data_source = models.DataSource.objects.get(
                data_name=validated_request_data["data_name"], bk_tenant_id=validated_request_data["bk_tenant_id"]
            )
        else:
            raise ValueError(_("找不到请求参数，请确认后重试"))

        return data_source.to_json(with_rt_info=validated_request_data["with_rt_info"])
...
class FullCmdbNodeInfoResource(Resource):
    ...
    def perform_request(self, validated_request_data: dict[str, Any]) -> None:
        ...
        from bk_monitor_base.metadata.task.tasks import create_full_cmdb_level_data_flow
...
class LabelResource(Resource):
    ...
    @final
    class RequestSerializer(serializers.Serializer):
        include_admin_only = serializers.BooleanField(default=False, label="是否展示管理员可见标签")
...
class GetEventGroupResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        bk_tenant_id = TenantIdField(label="租户ID")
        event_group_id = serializers.IntegerField(required=True, label="事件分组ID")
        with_result_table_info = serializers.BooleanField(required=False, label="是否需要带结果表信息", default=False)
...
class QueryTimeSeriesScopeResource(Resource):
    ...
    def _build_grouped_results(query_set, group_id, include_metrics):
        ...
        # 预聚合每个 scope 的 metric 数量，避免在列表推导中产生 N+1 查询
        metric_count_map: dict[int, int] = dict(
            models.TimeSeriesMetric.objects.filter(group_id=group_id, scope_id__in=scope_ids)
            .values("scope_id")
            .annotate(count=Count("id"))
            .values_list("scope_id", "count")
        )
...
class ListTransferClusterResource(Resource):
    ...
    def perform_request(self, validated_request_data: dict[str, Any]):
        ...
        black_transfer_cluster_id = settings.metadata.transfer_builtin_cluster_id or ""
...
class ListBCSResourceInfoResource(Resource):
    ...
    def perform_request(self, validated_request_data: dict[str, Any]):
        ...
        resources = []
        for res_type in resource_type.split(","):
            if not res_type:
                continue

            resource_cls = type_to_class.get(res_type)
            if resource_cls is None:
...
class KafkaTailResource(Resource):
    ...
    @override
    def perform_request(self, validated_request_data: dict[str, Any]) -> list[Any]:
        bk_tenant_id = validated_request_data["bk_tenant_id"]
        ...
        if mq_ins.is_auth:
            result: list[Any] = self._consume_with_confluent_kafka(mq_ins, datasource, size)
        # 是否是V4数据链路
    ...
    def _consume_with_confluent_kafka(
        self, mq_ins: models.ClusterInfo, datasource: models.DataSource, size: int
    ) -> list[Any]:
        """
    ...
    def _consume_with_kafka_python(
        self, datasource: models.DataSource, mq_ins: models.ClusterInfo, size: int
    ) -> list[Any]:
        """
        ...
        if mq_ins.is_ssl_verify:  # SSL验证是否强验证
            ...
            consumer = KafkaConsumer(
                ...
                request_timeout_ms=settings.metadata.kafka_tail_api_timeout_seconds,
                consumer_timeout_ms=settings.metadata.kafka_tail_api_timeout_seconds,
        ...
        max_retries = settings.metadata.kafka_tail_api_retry_times
        retry_delay = settings.metadata.kafka_tail_api_retry_interval_seconds
    ...
    def _consume_with_gse_config(self, datasource: models.DataSource, size: int) -> list[Any]:
        """
        ...
        route_info_list = api.gse.query_route(bk_tenant_id=DEFAULT_TENANT_ID, **route_params)
        stream_to_id = None
        topic = None
        ...
        kafka_params: dict[str, dict[str, Any]] = {
            "condition": {
        ...
        kafka_config_list = api.gse.query_stream_to(bk_tenant_id=DEFAULT_TENANT_ID, **kafka_params)
...
class SyncBkBaseRtMetaByBizIdResource(Resource):
    ...
    def perform_request(self, validated_request_data: dict[str, Any]):
        ...
        bk_biz_id = validated_request_data["bk_biz_id"]
        # 同步指定存储类型的数据
        storages = settings.metadata.sync_bkbase_meta_supported_storage_types
        logger.info("SyncBkBaseRtMetaByBizIdResource: start sync bkbase meta for biz_id->[%s]", bk_biz_id)
        bkbase_rt_meta_list = api.bkdata.bulk_list_result_table(
            bk_tenant_id=DEFAULT_TENANT_ID, bk_biz_id=[bk_biz_id], storages=storages
        )
...
class MetadataAPI:
    query_tag_values: Resource = QueryTagValuesResource()
    list_label: Resource = LabelResource()
    delete_event_group: Resource = DeleteEventGroupResource()
    get_event_group: Resource = GetEventGroupResource()
    delete_time_series_group: Resource = DeleteTimeSeriesGroupResource()
    get_time_series_group: Resource = GetTimeSeriesGroupResource()
    kafka_tail: Resource = KafkaTailResource()
    query_time_series_group: Resource = QueryTimeSeriesGroupResource()
    create_result_table_metric_split: Resource = CreateResultTableMetricSplitResource()
    full_cmdb_node_info: Resource = FullCmdbNodeInfoResource()
    list_result_table: Resource = ListResultTableResource()
    get_result_table: Resource = QueryResultTableSourceResource()
    get_data_source: Resource = QueryDataSourceResource()
    modify_event_group: Resource = ModifyEventGroupResource()
    create_event_group: Resource = CreateEventGroupResource()
    create_time_series_group: Resource = CreateTimeSeriesGroupResource()
    modify_time_series_group: Resource = ModifyTimeSeriesGroupResource()
    modify_result_table: Resource = ModifyResultTableResource()
    create_result_table: Resource = CreateResultTableResource()
    modify_data_source: Resource = ModifyDataSource()
    create_data_source: Resource = CreateDataIDResource()
    query_event_group: Resource = QueryEventGroupResource()
    query_cluster_info: Resource = QueryClusterInfoResource()
    list_transfer_cluster: Resource = ListTransferClusterResource()
    get_result_table_storage: Resource = GetResultTableStorageResult()
    create_cluster_info: Resource = CreateClusterInfoResource()
    modify_cluster_info: Resource = ModifyClusterInfoResource()
    delete_cluster_info: Resource = DeleteClusterInfoResource()
