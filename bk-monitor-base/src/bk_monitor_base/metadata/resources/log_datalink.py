...
from bk_monitor_base.metadata.service.space_redis import push_and_publish_es_aliases
...
from bk_monitor_base.metadata.utils.user import get_request_username
...
class BaseLogRouter(Resource, metaclass=abc.ABCMeta):
    def create_or_update_options(self, bk_tenant_id: str, table_id: str, options: list[dict[str, Any]]):
        """创建或者更新结果表 option"""
        # 查询结果表下的option
        exist_objs = {
            obj.name: obj
...
class BulkCreateOrUpdateLogRouter(BaseLogRouter):
    ...
    def perform_request(self, validated_request_data: dict[str, Any]):
        ...
        space: models.Space = models.Space.objects.get(
            space_type_id=space_type, space_id=space_id, bk_tenant_id=bk_tenant_id
        )
        logger.info(
    ...
    def _push_routes(
        ...
        if space_type != "bkcc":
            related_space = get_related_spaces(
                space_type_id=space_type, space_id=space_id, target_space_type_id=SpaceTypes.BKCC.value
            )
            if related_space:
                SpaceTableIDRedis().push_space_table_ids(
                    space_type=SpaceTypes.BKCC.value, space_id=related_space[0].space_id, is_publish=True
                )
