...
from bk_monitor_base.metadata.config import METADATA_RESULT_TABLE_WHITE_LIST, settings
from bk_monitor_base.metadata.models import AccessVMRecord, DataSource
...
class QueryDataLinkInfoResource(Resource):
    ...
    def _get_data_id_details(self, ds: models.DataSource) -> dict[str, Any]:
        """
        组装数据源详情信息
        """
    ...
    def _get_table_ids_details(self, bk_tenant_id: str, table_ids: list[str]) -> dict[str, Any]:
        """
    ...
    def _get_bkbase_details(self, bk_tenant_id: str) -> list[dict[str, Any]]:
        """
        根据table_ids，批量获取计算平台结果表详情信息
        """
        bkbase_details: list[dict[str, Any]] = []
    ...
    def _get_es_storage_details(self, bk_tenant_id: str) -> dict[str, dict[str, Any]]:
        """
        ...
        table_ids_details: dict[str, dict[str, Any]] = {}
    ...
    def _check_expired_metrics(self, bk_tenant_id: str, bk_data_id: str) -> list[dict[str, Any]]:
        """
        针对时序指标类型，检查其是否存在指标过期问题
        """
        expired_metric_infos: list[dict[str, Any]] = []
        logger.info("QueryDataLinkInfoResource: start to check expired metrics")
        try:
            now = timezone.now()
            expired_time = now - timedelta(seconds=settings.metadata.time_series_metric_expired_seconds)
    ...
    def _get_authorized_space_uids(self, bk_tenant_id: str, bk_data_id: str) -> list[str]:
        """
    ...
    def _check_result_table_detail_metric_router_status(self, bk_tenant_id: str) -> list[dict[str, Any]]:
        """
        检查结果表指标路由
        若在Transfer/计算平台侧存在指标，但是在RESULT_TABLE_DETAIL中不存在，说明路由异常
        """
        rt_detail_router_infos: list[dict[str, Any]] = []
        for table_id in self.time_series_table_ids:
            try:
    ...
    def _check_space_to_result_table_router(
        ...
        """
        space_to_result_table_router_infos: dict[str, list[dict[str, Any]]] = {}
...
class IntelligentDiagnosisMetadataResource(Resource):
    ...
    def perform_request(self, validated_request_data: dict[str, Any]) -> Any:
        from bk_monitor_base.metadata.agents.diagnostic.metadata_diagnostic_agent import MetadataDiagnosisAgent  # noqa
...
class GseSlotResource(Resource):
    ...
    @override
    def perform_request(self, validated_request_data: dict[str, str]) -> bool:
        if not settings.metadata.gse_slot_id or not settings.metadata.gse_slot_token:
            logger.warning("GseSlotResource: gse slot id or token is not set, skip")
            return False

        from bk_monitor_base.metadata.task.tasks import process_gse_slot_message
...
class QueryDataLinkMetadataResource(Resource):
        ...
        def validate(self, attrs: dict[str, Any]) -> dict[str, Any]:
            ...
            if not any([bk_data_id, result_table_id, vm_result_table_id]):
                raise serializers.ValidationError(
                    "At least one of 'bk_data_id', 'result_table_id', or 'vm_result_table_id' must be provided. 至少需要提供 'bk_data_id'、'result_table_id' 或 'vm_result_table_id' 中的一个。"
                )
    ...
    def _resolve_bk_data_id(
        ...
        if result_table_id:
            try:
                ...
                if not dsrt:
                    raise ValidationError(
                        f"Result table '{result_table_id}' not found in DataSourceResultTable. 结果表 '{result_table_id}' 在 DataSourceResultTable 中未找到。"
                    )
                return {
        ...
        if vm_result_table_id:
            try:
                ...
                if not vm_record:
                    raise ValidationError(
                        f"VM result table '{vm_result_table_id}' not found in AccessVMRecord. VM结果表 '{vm_result_table_id}' 在 AccessVMRecord 中未找到。"
                    )
                ...
                if not dsrt:
                    raise ValidationError(
                        f"Result table '{vm_record.result_table_id}' (from VM record) not found in DataSourceResultTable. 结果表 '{vm_record.result_table_id}'（来自VM记录）在 DataSourceResultTable 中未找到。"
                    )
        ...
        raise ValidationError(
            "Unable to resolve bk_data_id. Please provide at least one of: bk_data_id, result_table_id, vm_result_table_id. 无法解析 bk_data_id。请至少提供以下参数之一：bk_data_id、result_table_id、vm_result_table_id。"
        )
    ...
    def _get_space_info(self, bk_tenant_id: str, bk_biz_id: int) -> dict[str, str]:
        ...
        try:
            if bk_biz_id > 0:
                space: models.Space = models.Space.objects.get(
                    bk_tenant_id=bk_tenant_id, space_type_id=SpaceTypes.BKCC.value, space_id=bk_biz_id
                )
            elif bk_biz_id < 0:
                space: models.Space = models.Space.objects.get(bk_tenant_id=bk_tenant_id, id=abs(bk_biz_id))
            else:
