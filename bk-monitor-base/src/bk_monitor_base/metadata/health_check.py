...
from bk_monitor_base.metadata.models import BCSClusterInfo, DataSource, DataSourceResultTable, ResultTable
...
from bk_monitor_base.metadata.models.data_link.service import get_data_id_v2
from bk_monitor_base.metadata.models.data_link.utils import compose_bkdata_data_id_name
    ...
    for result_table in result_tables:
        ...
        if result_table.data_label:
            ...
            for data_label in data_labels:
                data_label_key = (
                    f"{data_label}|{bk_tenant_id}" if get_config().blueking.enable_multi_tenancy else data_label
                )
                data_label_result_table_ids_raw = RedisTools.hget(DATA_LABEL_TO_RESULT_TABLE_KEY, data_label_key)
                data_label_result_table_ids: list[str] = (
                    json.loads(data_label_result_table_ids_raw) if data_label_result_table_ids_raw else []
                )
        ...
        # 检查结果表详情是否存
        result_table_detail_key = (
            f"{result_table.table_id}|{bk_tenant_id}"
            if settings.blueking.enable_multi_tenancy
            else result_table.table_id
        )
...
def get_datalink_status_by_scene(
    *,
    ...
    if scene == DataScene.HOST and settings.blueking.space_builtin_data_link_mode == "tenant":
