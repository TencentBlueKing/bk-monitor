...
def get_client_by_datasource_info(datasource_info: dict[str, Any]):
    ...
    # 获取超时时间
    timeout_config = settings.metadata.metadata_request_es_timeout
...
# 保留 bytes 关键字，兼容既有调用以及 Elasticsearch 同名参数。
def get_cluster_disk_size(es_client, kind="total", bytes="b"):  # noqa: A002
