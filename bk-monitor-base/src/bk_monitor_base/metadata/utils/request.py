...
def get_request_tenant_id(peaceful: bool = False) -> str | None:
    ...
    获取当前请求的租户id
    """
    from bk_monitor_base.metadata.utils.tenant import get_local_tenant_id
...
def set_request(request: HttpRequest):
    ...
    设置当前请求
    """
    from bk_monitor_base.metadata.utils.tenant import set_local_tenant_id
    from bk_monitor_base.metadata.utils.user import set_local_username
...
def get_app_code_by_request(request) -> str:
    bk_app_code: str | None = request.META.get("HTTP_BK_APP_CODE")
    if bk_app_code is None and hasattr(request, "jwt"):
        bk_app_code: str = request.jwt.app.app_code

    bk_app_code = bk_app_code or settings.common.default_source_system
    return bk_app_code
