...
from bk_monitor_base.domains.metric_plugin.manager.job.define import TASK_DEBUG_STEP
from bk_monitor_base.domains.metric_plugin.manager.job.mysql import MysqlPluginManager
from bk_monitor_base.domains.metric_plugin.manager.job.task import DebugContext, ParseDebugMetrics
...
class TestMysqlPluginManager:
    """测试 MysqlPluginManager 类"""
    ...
    def setup_method(self):
        ...
        self.plugin_manager = MysqlPluginManager(self.plugin)

    def test_get_binary_path(self):
        """测试获取二进制文件路径"""
        # 测试 Linux 路径
        path_linux = self.plugin_manager.get_binary_path(OSType.LINUX)
        assert path_linux == "job_plugin/job_mysql/OracleUniversalPlugin"

        # 测试 Linux AArch64 路径
        path_arm = self.plugin_manager.get_binary_path(OSType.LINUX_AARCH64)
        assert path_arm == "job_plugin/job_mysql/OracleUniversalPlugin_arm"

        # 测试不支持的系统类型
        with pytest.raises(ParseOsTypeError):
            self.plugin_manager.get_binary_path(OSType.WINDOWS)

    def test_find_plugin_dir_matches_job_plugins_dir(self, tmp_path: Path):
        """测试导入作业插件包时匹配 job_plugins 操作系统目录。"""
        invalid_plugin_dir = tmp_path / "external_plugins_linux_x86_64" / "bkplugin_exporter"
        invalid_plugin_dir.mkdir(parents=True)
        expected_plugin_dir = tmp_path / "job_plugins_linux_x86_64" / "bkplugin_mysql"
        expected_plugin_dir.mkdir(parents=True)

        plugin_dir, plugin_name = MysqlPluginManager._find_plugin_dir(tmp_path)

        assert plugin_dir == expected_plugin_dir
        assert plugin_name == "bkplugin_mysql"

    def test_find_plugin_dir_ignores_non_job_plugins_dir(self, tmp_path: Path):
        """测试作业插件导入不误匹配节点管理插件目录。"""
        invalid_plugin_dir = tmp_path / "external_plugins_linux_x86_64" / "bkplugin_exporter"
        invalid_plugin_dir.mkdir(parents=True)

        with pytest.raises(ValueError, match="插件包中未找到有效的插件目录"):
            MysqlPluginManager._find_plugin_dir(tmp_path)

    def test_get_plugin_type_from_package_matches_job_plugins_dir(self, tmp_path: Path):
        """测试作业插件管理器按 job_plugins 目录识别插件类型。"""
        meta_file = tmp_path / "job_plugins_linux_x86_64" / "bkplugin_mysql" / "info" / "meta.yaml"
        meta_file.parent.mkdir(parents=True)
        meta_file.write_text("plugin_type: job_mysql\n", encoding="utf-8")

        plugin_type = MysqlPluginManager.get_plugin_type_from_package(tmp_path)

        assert plugin_type == "job_mysql"
    ...
    def test_make_package_creates_non_empty_tgz(self, tmp_path: Path):
        """测试制作插件包会生成包含文件的压缩包"""
        binary_path = tmp_path / "OracleUniversalPlugin"
        binary_path.write_bytes(b"binary_content")
        extra_files = {
            OSType.LINUX: [
                {"source_path": str(binary_path), "target_path": "OracleUniversalPlugin"},
            ],
            OSType.LINUX_AARCH64: [
                {"source_path": str(binary_path), "target_path": "OracleUniversalPlugin_arm"},
            ],
        }

        package_path = self.plugin_manager._make_package(extra_files=extra_files, is_compress=True)

        assert package_path.stat().st_size > 68
        with tarfile.open(package_path, "r:gz") as tar:
            file_names = [member.name for member in tar.getmembers() if member.isfile()]
        assert file_names
        assert any(file_name.endswith("OracleUniversalPlugin") for file_name in file_names)
        assert any(file_name.endswith("project.yaml") for file_name in file_names)

    def test_make_package_includes_sql_content_info_file(self, mocker: MockerFixture, tmp_path: Path):
        """测试制作插件包时将 SQL 定义写入 info 目录。"""
        binary_path = tmp_path / "OracleUniversalPlugin"
        binary_path.write_bytes(b"binary_content")
        mocker.patch.object(self.plugin_manager, "_download_binary", return_value=binary_path)

        package_path = self.plugin_manager.make_package(is_compress=True)

        with tarfile.open(package_path, "r:gz") as tar:
            sql_content_member = next(
                member for member in tar.getmembers() if member.name.endswith("info/sql_content.json")
            )
            sql_content_file = tar.extractfile(sql_content_member)
            assert sql_content_file is not None
            sql_content = json.load(sql_content_file)

        assert sql_content == self.plugin.define["sql_content"]

    def test_parse_define_reads_sql_content_info_file(self, tmp_path: Path):
        """测试导入插件包时从 info/sql_content.json 还原 SQL 定义。"""
        info_dir = tmp_path / "job_plugins_linux_x86_64" / "test_plugin" / "info"
        info_dir.mkdir(parents=True)
        sql_content = [
            {
                "classification_id": "sql1",
                "classification_name": "SQL-111",
                "content": "SELECT '{{ raw }}' as metrics_test;",
            }
        ]
        (info_dir / "sql_content.json").write_text(json.dumps(sql_content), encoding="utf-8")

        define = MysqlPluginManager._parse_define(
            bk_tenant_id="test_tenant",
            operator="admin",
        ...
        assert define == {"sql_content": sql_content}

    def test_create_compress_package_raises_when_package_dir_empty(self, tmp_path: Path):
        """测试空插件目录不会生成空压缩包"""
        plugin_dir = tmp_path / "empty_plugin"
        plugin_dir.mkdir()

        with pytest.raises(ExportPluginFailedError, match="插件包目录为空"):
            self.plugin_manager._create_compress_package(plugin_dir)

    def test_export_package_returns_bkrepo_download_url(self, mocker: MockerFixture):
        """测试导出插件包返回 repo 下载 URL"""
        self.plugin.status = MetricPluginStatus.RELEASE
        self.plugin.related_params = {
            self.plugin_manager.PLUGIN_REPO_TAR_PATH_KEY: "job_plugins/test_plugin/123/test_plugin.tgz"
        }
        mock_storage = mocker.Mock()
        mock_storage.url.return_value = "http://bkrepo.example.com/job_plugins/test_plugin/123/test_plugin.tgz"
        mock_get_storage = mocker.patch(
            "bk_monitor_base.domains.metric_plugin.manager.job.base.get_storage",
            return_value=mock_storage,
        )

        download_url = self.plugin_manager.export_package(operator="admin")

        assert download_url == "http://bkrepo.example.com/job_plugins/test_plugin/123/test_plugin.tgz"
        mock_get_storage.assert_called_once_with(StorageName.JOB)
        mock_storage.url.assert_called_once_with("job_plugins/test_plugin/123/test_plugin.tgz")

    def test_export_package_raises_when_plugin_not_released(self):
        """测试未发布插件不能导出"""
        self.plugin.related_params = {
            self.plugin_manager.PLUGIN_REPO_TAR_PATH_KEY: "job_plugins/test_plugin/123/test_plugin.tgz"
        }

        with pytest.raises(ExportPluginFailedError):
            self.plugin_manager.export_package(operator="admin")

    def test_export_package_raises_when_repo_path_missing(self):
        """测试发布后未生成 repo 路径时导出超时"""
        self.plugin.status = MetricPluginStatus.RELEASE
        self.plugin.related_params = {}
    ...
    def test_generate_config_yaml_content_casts_numeric_params_to_int(self):
        """测试生成配置时将数字字符串转为整数"""
        config = self.plugin_manager.generate_config_yaml_content(
            plugin_params={
                "host": "10.10.28.200",
                "name": "bk_monitor",
                "password": "secret",
        ...
        assert config["session"] == 10
        assert config["time"] == 60

    def test_generate_config_yaml_content_raises_when_numeric_param_invalid(self):
        """测试数字字段非法时提前报错"""
        with pytest.raises(ValueError, match="port"):
            self.plugin_manager.generate_config_yaml_content(plugin_params={"port": "3306/tcp"})
    ...
    def test_start_debug(self, mocker: MockerFixture):
        ...
        assert context["binary_path"] == "job_plugin/job_mysql/OracleUniversalPlugin"
    ...
    def test_generate_debug_yaml(self, mocker: MockerFixture):
        ...
        # 验证返回路径
        assert remote_path == "job_plugin/job_mysql/debug_123/sql_config.yml"
    ...
    def test_get_debug_log(self, mocker: MockerFixture):
        ...
        mock_debug_inst.job_type = "job_mysql"
        mock_debug_inst.debug_params = {"collect_host": {"bk_host_id": 1}}
    ...
    def test_parse_debug_metrics_rejects_reserved_field_name(self, mocker: MockerFixture):
        """SQL调试返回列名命中保留字时，应记录准确日志并标记失败。"""
        debug_inst = mocker.MagicMock()
        ctx = DebugContext(
            debug_task_inst_id=123,
            job_debug_inst=debug_inst,
            plugin=self.plugin,
            current_step=TASK_DEBUG_STEP.PARSE_DEBUG_METRICS,
        )
        context = {
            "sql_content": [
                {
                    "classification_id": "sql1",
                    "classification_name": "SQL-111",
                    "content": "SELECT time, value FROM metrics;",
                }
            ]
        }
        sql_json_str = json.dumps(
            [
                {
                    "sql": "SELECT time, value FROM metrics;",
                    "data": [{"time": "2026-05-18 12:00:00", "value": 1}],
                }
            ]
        )

        with pytest.raises(ParsePluginDebugContentError, match="字段\[time\]命中"):
            ParseDebugMetrics._parse_metrics(sql_json_str, context, ctx)

        debug_inst.add_log_entry.assert_called_once()
        log_message = debug_inst.add_log_entry.call_args.kwargs["messages"]
        assert "字段[time]命中 InfluxDB/结果表保留字" in log_message
        assert "SQL规则：sql1(SQL-111)" in log_message
        assert "使用 AS 为该列设置非保留字别名" in log_message
        debug_inst.failed.assert_called_once()
        assert "time_value" in debug_inst.failed.call_args.args[0]
    ...
    def test_get_supported_os_types_uses_binary_paths_when_define_has_no_os_markers(self):
        ...
        assert supported_types == list(MysqlPluginManager.BINARY_PATHS)

    def test_get_supported_os_types_matches_binary_paths_order(self):
        """测试支持系统列表与二进制路径声明顺序保持一致"""
        assert self.plugin_manager.get_supported_os_types() == list(MysqlPluginManager.BINARY_PATHS)
    ...
    def test_get_parse_rules_json_with_metrics(self):
        ...
        manager = MysqlPluginManager(plugin_with_metrics)
    ...
    def test_get_package_context(self):
        ...
        assert context["plugin_display_name"] == "测试MySQL插件"
    ...
    def test_get_debug_os_target_path(self, mocker: MockerFixture):
        ...
        assert "/data/bkce/gse/job_plugins/debug_job_mysql_123/test_plugin" in linux_path

        windows_path = self.plugin_manager._get_debug_os_target_path(123, OSType.WINDOWS)
        assert "C:\\gse\\job_plugins\\debug_job_mysql_123\\test_plugin" in windows_path
    ...
    def test_get_host_info_missing_os_type_queries_cmdb_by_host_id(self, mocker: MockerFixture):
        """测试获取主机信息 - 缺少os_type时可按主机ID回查CMDB"""
        ...
        assert host_info["bk_host_id"] == 1
        assert host_info["ip"] == "10.0.0.1"
        assert host_info["os_type"] == "linux"
        assert host_info["os_arch"] == "x86_64"
        assert mock_search.call_args.kwargs["bk_inst_ids"] == [1]

    def test_get_host_info_missing_os_arch_queries_cmdb_by_ip_and_cloud(self, mocker: MockerFixture):
        """测试获取主机信息 - 缺少os_arch时可按IP和云区域回查CMDB"""
        mocker.patch(
            "bk_monitor_base.domains.metric_plugin.mock_cmdb_tools.search_instances",
            side_effect=[
                (0, []),
                    ...
                    [
                        {
                            "bk_inst_id": 2,
                            "bk_cloud_id": 0,
                            "bk_host_innerip": "10.100.244.241",
                            "bk_os_type": "linux",
                            "bk_cpu_architecture": "aarch64",
                        }
                    ],
                ),
            ],
        ...
        bk_host = {"ip": "10.100.244.241", "bk_cloud_id": 0, "os_type": "linux"}

        host_info = get_host_info(bk_host)

        assert host_info["bk_host_id"] == 2
        assert host_info["ip"] == "10.100.244.241"
        assert host_info["os_type"] == "linux"
        assert host_info["os_arch"] == "aarch64"

    def test_get_os_type_by_collect_host_returns_none_when_host_info_incomplete(self, mocker: MockerFixture):
        """测试主机信息不完整时返回None，而不是抛出AttributeError"""
        mocker.patch(
            "bk_monitor_base.domains.metric_plugin.mock_cmdb_tools.get_host_info",
            return_value={"os_type": None, "os_arch": None},
        )

        collect_host = {"ip": "10.100.244.241", "bk_cloud_id": 0}
    ...
    def test_type_class_variable(self):
        """测试类型类变量"""
        assert MysqlPluginManager.type == "job_mysql"

    def test_binary_paths(self):
        """测试二进制文件路径配置"""
        assert OSType.LINUX in MysqlPluginManager.BINARY_PATHS
        assert MysqlPluginManager.BINARY_PATHS[OSType.LINUX] == "OracleUniversalPlugin"

        assert OSType.LINUX_AARCH64 in MysqlPluginManager.BINARY_PATHS
        assert MysqlPluginManager.BINARY_PATHS[OSType.LINUX_AARCH64] == "OracleUniversalPlugin_arm"
