...
setup() {
  ...
  const getTimeInMs = (timeValue: string): number => {
    ...
    const amount = parseInt(timeValue.slice(0, -1));

    // 输入验证
    if (isNaN(amount) || amount <= 0) {
      console.warn(`Invalid time amount: ${timeValue}`);
      return 0;
    }
    
    if (!unitMap[unit]) {
      console.warn(`Invalid time unit: ${unit} in ${timeValue}`);
      return 0;
    }
  ...
  render() {
    return (
      <div class="auto-refresh">
        <span class="bklog-icon bklog-auto-refresh" style="font-size:20px;margin:0 5px 0 5px"></span>
        <bk-select
          disabled={false}
        ...
        <span class="icon bklog-icon bklog-refresh2" style="font-size:20px;margin:0 5px 0 5px;cursor:pointer" onClick={this.doRefresh}></span>
      </div>
    );
  }
});