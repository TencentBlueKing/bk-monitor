<script setup lang="ts">
  ...
  const isExactOperator = (operator: string) =>
    ['contains match phrase', 'not contains match phrase'].includes(operator);
  ...
  const inferModeAndKeywords = (
    value: string | string[],
    operator = props.operator,
  ): { mode: Mode; keywords: string[] } => {
  ...
  const actualQueryList = computed(() =>
    keywordList.value.map(item => computeQuery(activeMode.value, item)).filter(Boolean),
  );
  const actualQueryText = computed(() =>
    actualQueryList.value.length ? actualQueryList.value.join(` ${currentRelation.value} `) : '',
  );
