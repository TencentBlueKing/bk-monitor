<script setup lang="ts">
  ...
  // @ts-ignore
  import {
    formatDateTimeField as formatDateTimeValue,
    getCharLength,
    getOsCommandLabel,
    getRegExp,
  } from '@/common/util';
  ...
  const handleBatchInputChange = selectData => {
    condition.value.value = [...new Set([...(condition.value.value || []), ...selectData])];
  };
  ...
  const isFuzzyMatchAvailable = computed(() => {
    return (
      ['text', 'string'].includes(activeFieldItem.value.field_type) &&
  ...
  const fuzzyMatchEngine = computed(() => {
    const indexSetIds = store.getters.isUnionSearch ? store.getters.unionIndexList : [store.getters.indexId];
    const selectedIndexSetList = store.state.retrieve.flatIndexSetList.filter(
      item => indexSetIds.includes(String(item.index_set_id)) || indexSetIds.includes(item.index_set_id),
    );
  ...
  const restoreFieldAndCondition = () => {
    ...
    let filterIndex = filterFieldList.value.findIndex(
      (field: any) =>
        field.field_name === activeFieldItem.value.field_name && field.field_type === activeFieldItem.value.field_type,
    );
    ...
    () => {
      nextTick(() => {
        ...
        if (typeof props.value === 'object' && props.value !== null && (props.value as any).field) {
          ...
          const valueField = (props.value as any).field;
          const filterIndex = filterFieldList.value.findIndex((field: any) => field.field_name === valueField);
          if (filterIndex !== -1) {
            activeIndex.value = filterIndex;
          }
          return;
  ...
  let tagInputTimer: any | null = null;
  ...
  const handleConditionValueInputFocus = _e => {
    isConditionValueInputFocus.value = true;
    conditionBlurTimer && clearTimeout(conditionBlurTimer);
    conditionBlurTimer = null;
  ...
  const beforeShowndFn = () => {
    if (!isMountedEventAdded) {
      ...
      if (
        activeIndex.value === null ||
        (typeof props.value === 'object' && props.value !== null && !(props.value as any).field)
      ) {
        setDefaultActiveIndex();
      }
  ...
  const handleOptionListMouseEnter = (e, item) => {
    ...
    if (offsetWidth < scrollWidth) {
      fieldOptionListInstance.setContent(`${item.query_alias || item.field_name}(${item.field_name})`);
