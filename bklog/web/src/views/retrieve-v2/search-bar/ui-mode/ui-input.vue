<script setup>
  ...
  const customPlaceholderStyle = computed(() =>
    getCustomPlaceholderStyle(isCustomPlaceholderEmpty.value, paddingLeft.value),
  );
  ...
  const handleSaveQueryClick = payload => {
    ...
    const targetValue = formatModelValueItem(isFulltextEnterVlaue ? getInputQueryDefaultItem(inputVal) : payload);
              ...
              <div class="match-value-content">
                ...
                >
                  <div
                    ...
                    :class="[{ 'delete-line': item.hidden_values?.includes(child) }]"
                  >
                    {{ child }}
                  </div>
                  <div slot="content">
