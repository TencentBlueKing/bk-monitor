  ...
  <div ref="refRootElement" :class="['search-bar-wrapper']">
    <div :class="[
      ...
      <div class="search-input" :class="{ disabled: isInputLoading }">
        <UiInput class="search-input-section" v-if="activeIndex === 0" v-model="uiQueryValue"
        ...
        <SqlQuery class="search-input-section" v-if="activeIndex === 1" v-model="sqlQueryValue"
          @retrieve="handleSqlRetrieve" @change="handleSqlQueryChange">
          <template #custom-placeholder="{ isEmptyText }">
            <slot name="custom-placeholder" :isEmptyText="isEmptyText"></slot>
          </template>
        </SqlQuery>
        ...
        <div class="search-tool items" v-if="isShowSearchTools">
          ...
          <div v-show="!inspectResponse.is_legal" style="color: #ea3636"
            class="bklog-icon bklog-circle-alert-filled bklog-keyword-validate" @mouseenter="handleMouseenterInspect"
            @mouseleave="handleMouseleaveInspect">
            <div v-show="false">
              <div class="bklog-keyword-inspect" ref="refKeywordInspectElement"
                @mouseenter="(e) => handleMouseenterInspect(e, false)" @mouseleave="handleMouseleaveInspect">
                ...
                <div class="inspect-row" v-show="inspectResponse.is_resolved">
                  <div class="inspect-title">
                    <span>{{ $t("你可能想输入:") }}</span><span class="inspect-keyword-replace"
                      @click="handleInspectKeywordReplace">{{ $t("替换") }}</span>
                  </div>
                  <div class="inspect-message">
          ...
          <div v-if="showCopy" v-bk-tooltips="$t('复制当前查询')" :class="[
            'bklog-icon bklog-copy-4',
            ,
            { disabled: isInputLoading || !isCopyBtnActive },
          ]" @click="handleCopyQueryValue"></div>
          <div v-if="showClear" v-bk-tooltips="$t('清理当前查询')" :class="[
            'bklog-icon bklog-qingkong',
            { disabled: isInputLoading || !isCopyBtnActive },
          ]" @click="handleClearBtnClick"></div>
          <div v-if="showQuerySetting" v-bk-tooltips="$t('常用查询设置')" :class="[
            'bklog-icon bklog-setting',
            { disabled: isInputLoading, 'is-focused': isFilterSecFocused },
          ]" @click="handleFilterSecClick" />
          <BookmarkPop v-if="showFavorites" :active-favorite="!activeFavorite?.id" :addition="uiQueryValue"
            :class="{ disabled: isInputLoading }" :match-s-q-l-str="matchSQLStr"
            :search-mode="SEARCH_MODE_DIC[activeIndex]" :sql="sqlQueryValue" @refresh="handleRefresh"
            @save-current-active-favorite="saveCurrentActiveFavorite" />
        </div>
        <div class="search-tool search-btn" @click="handleBtnQueryClick">
          <i :class="isSearching ? 'bklog-icon bklog-zanting' : 'bklog-icon bklog-shoudongchaxun'" />
