<script setup>
  ...
  const handleCreateRequest = async () => {
    ...
    const param = {
      filterSetting: {
        switchingMode: SettingData.value.switchingMode,
  ...
  const showPopover = async () => {
    ...
    await store.dispatch('requestIndexSetCustomConfigInfo').then(res => {
      const { switchingMode, filterFields } = userSettingConfig?.value?.filterSetting;
...
<template>
  ...
  >
    <span
      ...
      class="bklog-icon bklog-set-icon"
    ...
    <template #content>
      <div>
        ...
        >
          ...
          <bk-form-item
            :property="'switchingMode'"
            :label="$t('自动清空条件')"
          >
            <bk-radio-group v-model="SettingData.switchingMode">
              <bk-radio-button :value="true"> {{ $t('清空') }} </bk-radio-button>
              <bk-radio-button :value="false"> {{ $t('不清空') }} </bk-radio-button>
            </bk-radio-group>
          </bk-form-item>
        </bk-form>
      </div>
