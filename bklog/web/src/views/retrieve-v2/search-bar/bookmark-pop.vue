...
const handleCreateRequest = async () => {
  ...
  const data = {
    ...
    ip_chooser:
      formatAddition.value.find((item) => item.field === "_ip-select_")?.value?.[0] ?? {},
    index_set_ids: [],
    index_set_names: [],
    ...searchParams,
  };
    ...
    <template #content>
      <div>
        ...
        <bk-form
          ...
          form-type="vertical"
        >
          <bk-form-item :property="'name'" label="收藏名称" required>
            <bk-input v-model="favoriteData.name" placeholder="请输入收藏名称"></bk-input>
          </bk-form-item>
          <bk-form-item :property="'project'" label="所属分组">
            <bk-select
          ...
          <bk-form-item label="索引集">
            <bk-input :value="indexSetName" readonly show-overflow-tooltips></bk-input>
          </bk-form-item>
