...
const favoriteGroupList = computed(() =>
  store.state.favoriteList.map((f) => f.favorites).flat()
);
...
const isSqlMode = (item) => {
  return (
    item.search_mode === 'sql' && !(item.params.chart_params?.type ?? false)
  );
};
  ...
  const filter = favoriteGroupList.value
    .filter((item) => {
      return (
        ...
        item.index_set_ids?.some((id) =>
          indexSetItemIdList.value.includes(`${id}`)
        )
      );
    })
    .filter((child) => {
      if (child.favorite_type === 'chart') {
        return (
          ...
          regExpStringList.value.every((reg) =>
            reg.test(child.params?.chart_params?.sql)
          )
        );
      }
...
<template>
  <div class="favorite-footer">
    ...
    <div :class="['favorite-query-list', { 'no-data': !favoriteList.length }]">
      <div v-if="favoriteList.length" class="query-list-title">
        {{ $t('联想到以下') }}
