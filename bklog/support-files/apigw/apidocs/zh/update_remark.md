...
编辑指定索引集下日志聚类 Pattern 的已有标注。只能编辑当前用户创建、且创建时间和原内容均匹配的标注。
...
| old_remark | string | 是 | 待编辑的原标注内容 |
| new_remark | string | 是 | 编辑后的新标注内容，不能为空 |
| create_time | int | 是 | 待编辑标注的创建时间，取自 `data.remark[]` 中对应条目的 `create_time`，单位为毫秒 |
...
url = "https://example.com/pattern/1001/update_remark/"
headers = {
...
```json
{
  ...
  "data": {
    ...
    "updated_at": "2026-08-03T08:35:00Z",
