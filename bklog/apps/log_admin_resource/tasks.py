...
def filter_runtime_logs(log_evidence: dict[str, Any], options: dict[str, Any] | None) -> dict[str, Any]:
    ...
    needles = keywords if case_sensitive else [item.lower() for item in keywords]
    scanned_lines = 0
    matched_lines = 0
    ranges = []
...
def _apply_runtime_log_filter(probes: dict[str, Any], options: dict[str, Any] | None) -> None:
    probe = probes.get("collector_logs")
