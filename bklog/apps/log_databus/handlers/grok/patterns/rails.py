...
PATTERNS = [
    ...
    {
        "name": "RCONTROLLER",
        "pattern": r"(?P<controller>[^#]+)#(?P<action>\w+)",
        "sample": "UsersController#show",
        "description": "匹配 Rails 控制器和动作（如 UsersController#show）",
    },
    {
        "name": "RAILS3HEAD",
        "pattern": r"""Started %{WORD:verb} "%{URIPATHPARAM:request}" for %{IPORHOST:clientip} at (?P<timestamp>%{YEAR}-%{MONTHNUM}-%{MONTHDAY} %{HOUR}:%{MINUTE}:%{SECOND} %{ISO8601_TIMEZONE})""",
    ...
    {
        "name": "RAILS3PROFILE",
        "pattern": r"(?:\((?:Views: %{NUMBER:viewms}ms \| )?ActiveRecord: %{NUMBER:activerecordms}ms)?",
        "sample": "(Views: 50ms | ActiveRecord: 30ms",
        "description": "匹配 Rails 3 性能分析信息（视图渲染时间和 ActiveRecord 查询时间）",
    },
    {
        "name": "RAILS3",
        "pattern": r"%{RAILS3HEAD}(?:%{RPROCESSING})?(?P<context>(?:%{DATA}\n)*)(?:%{RAILS3FOOT})?",
