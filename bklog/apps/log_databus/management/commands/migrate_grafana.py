...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        end_time = time.time()
        print("[migrate_grafana] #### END ####, Cost: %d s" % (end_time - start_time))
    ...
    def get_grafana_config(self, bklog_grafana_client, selected_biz):
        ...
        if orgs_resp.status_code != 200:
            self.stdout.write(f"failed: get config from bk_log: {orgs_resp.json()}")
    ...
    def extract_grafana_config(self, orgs):
        ...
        for org in orgs:
            ...
            if dashboards_resp.status_code != 200:
                self.stdout.write(f"FAILED to get dashboards for org {org_name}: {dashboards_resp.json()}")
    ...
    def check_org_name(self, org_id, org_name, failed_biz):
        if not org_name.isdigit():
            self.stdout.write(f"SKIP process org_id {org_id} -- org_name(biz_id) {org_name}: invalid biz_id")
    ...
    def process_dashboards(self, dashboards, org_id, org_name, failed_biz):
        ...
        for dashboard in dashboards:
            ...
            if dashboard_detail_resp.status_code != 200:
                ...
                self.stdout.write(f"failed to get dashboard info in biz {org_name}: {dashboard_detail_resp.json()}")
                continue
            ...
            dashboard_data = {
                ...
                "panels": panel_info,
                "refresh": dashboard_detail.get('dashboard', {}).get('refresh', '60s'),
    ...
    def convert_bklog_grafana_config(self, grafana_url, grafana_data):
        ...
        if orgs_resp.status_code != 200:
            self.stdout.write(f"failed to convert orgs for bk_log: {orgs_resp.json()}")
    ...
    def update_datasource_in_data(self, grafana_url, biz_id, data, biz_to_org_mapping, failed_biz):
        ...
        if not bk_log_datasource_uid:
            self.stdout.write(f"failed to get datasource for biz_id {biz_id}, will use default datasource")
            return
    ...
    def get_datasource_uid(self, grafana_url, org_id, failed_biz, biz_id):
        ...
        if bk_monitor_ds_resp.status_code != 200:
            self.stdout.write(f"failed to get datasources for org {org_id}: {bk_monitor_ds_resp.json()}")
    ...
    def create_organization(self, grafana_url, biz_id, failed_biz):
        ...
        if resp.status_code == 200:
            ...
            self.stdout.write(f"failed create organization for biz_id {biz_id}: {resp.json()}")
            return None
    ...
    def update_panel_datasource(self, panel, bk_log_datasource_uid):
        ...
        if 'datasource' in panel:
            ...
            self.stdout.write(f"panel {panel['title']} has no datasource")
    ...
    def write_config_to_grafana(self, grafana_url, grafana_data, biz_to_org_mapping):
        ...
        for data in grafana_data:
            ...
            if not org_id:
                self.stdout.write(f"write failed for biz_id {biz_id}: need to be created")
    ...
    def get_all_folders(self, grafana_url, org_id, biz_id):
        ...
        if all_folders_resp.status_code == 200:
            ...
            self.stdout.write(f"write failed for biz_id {biz_id}: failed to get all folders")
        ...
        if folder_title != "General":
            ...
            if resp.status_code == 200:
                ...
                if resp_copy.status_code == 200:
                    ...
                    self.stdout.write(
                        f"create folder failed in org {org_id} with folder_title" f" {folder_title}: {resp_copy.json()}"
                    )
                    return "", None
            else:
                self.record_error(
    ...
    def create_dashboard(self, grafana_url, biz_id, org_id, dashboard, folder_title, folder_uid, failed_biz):
        ...
        if resp.status_code == 200:
            ...
            self.stdout.write(
                f"create dashboard failed {dashboard_info['title']} in org {org_id} "
                f"with folder_title {folder_title}: {resp.json()}"
            )
