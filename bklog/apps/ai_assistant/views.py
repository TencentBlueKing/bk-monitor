...
from apps.ai_assistant.serializers import InterpretLogSerializer
...
class AIAssistantViewSet(APIViewSet):
    ...
    def interpret_log(self, request, *args, **kwargs):
        """
        @api {POST} /ai_assistant/interpret_log/ AI 解读日志
        @apiName AIAssistantInterpretLog
        @apiGroup AIAssistant
        @apiDescription AI 解读日志

        @apiParam {String} space_uid 空间ID，必填。
        @apiParam {Integer} index_set_id 索引集ID，必填。
        @apiParam {Object} log_data 日志内容，必填。
        @apiParam {Object} message 当前聊天输入内容，必填。
        ...
        """
        data = self.params_valid(InterpretLogSerializer)
