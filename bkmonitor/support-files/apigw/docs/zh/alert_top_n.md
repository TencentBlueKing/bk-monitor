...
1. **字段选择**：`fields` 参数支持多个字段同时统计，常用字段包括：
   - `bk_biz_id`：业务ID
...
2. **时间分片**：当 `need_time_partition=true` 时，系统会自动将时间范围分片处理，提高查询性能。

3. **权限控制**：只能查询有权限的业务数据，无权限的业务会被自动过滤。

4. **性能优化**：建议合理设置时间范围和 `size` 参数，避免查询过大的数据集。