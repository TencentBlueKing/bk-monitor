...
| 字段                   | 类型   | 描述                                            |
|----------------------|------|-----------------------------------------------|
| method               | str  | 请求方法，可选（"POST","GET","PUT"）默认"GET"            |
| url                  | str  | 回调地址                                          |
...
| need_poll            | bool | 是否需要轮询（默认true）                                |
| notify_interval      | int  | 通知间隔（单位秒，最小60，默认3600）                         |
| interval_notify_mode | str  | 通知模式（可选"increasing", "standard"，默认"standard"） |
