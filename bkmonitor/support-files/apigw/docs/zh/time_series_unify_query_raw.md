...
| time_alignment         | bool       | 否  | 是否保留最后一个数据点，默认值`true`                         |
| null_as_zero           | bool       | 否  | 是否将空值转换为0，默认值`false`                          |
| query_method           | str        | 否  | 查询方法，默认值`query_data`                          |
| unit                   | str        | 否  | 单位，默认为空字符串                                     |
| not_time_align         | bool       | 否  | 是否不对齐时间窗口，默认值`false`                          |
| with_metric            | bool       | 否  | 是否返回metric信息，默认值`false`                       |
