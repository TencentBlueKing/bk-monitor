...
| time_range | str        | 否  | 事件结束的时间范围，格式为：2020-02-26 00:00:00 -- 2020-02-28 23:59:59 |
| days       | int        | 否  | 查询最近几天内的时间，这个参数存在，time_range则失效                          |
| conditions | list[dict] | 否  | 查询条件                                                     |
| page       | int        | 否  | 第几页，不传则不分页，默认1                                           |
| page_size  | int        | 否  | 每页数量，默认100，最大5000                                        |
| fields     | list[str]  | 否  | 返回字段列表，不传则返回所有字段，可选值包括：`related_info` 等                  |
...
| event_id      | str  | 异常事件ID                                        |
| status        | str  | 事件状态，ABNORMAL(未恢复) CLOSED(已失效) RECOVERED(已恢复) |
| create_time   | str  | 产生事件的时间，格式为yyyy-mm-dd hh:mm:ss                |
| begin_time    | str  | 该事件关联的第一个异常点的创建时间，格式为yyyy-mm-dd hh:mm:ss      |
| end_time      | str  | 事件结束时间，格式为yyyy-mm-dd hh:mm:ss                 |
| target_key    | str  | 事件目标，不存在则为空字符串                                |
| p_event_id    | str  | 父事件ID，默认为空                                    |
| related_info  | str  | 关联信息（仅当fields包含`related_info`时返回）             |
...
| data.time               | int  | 异常点的时间戳（Unix时间戳，单位：秒） |
| dimension_translation   | dict | 维度展示信息                |
| anomaly                 | dict | 异常信息                  |
| anomaly.key             | str  | 告警级别                  |
| anomaly.anomaly_message | str  | 异常描述                  |
| anomaly.anomaly_time    | str  | 异常时间                  |
| anomaly.anomaly_id      | str  | 异常地点ID                |
