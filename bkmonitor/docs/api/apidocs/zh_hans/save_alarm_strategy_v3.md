...
| 字段 | 类型 | 必选 | 默认值 | 描述 |
|---|---|---|---:|---|
| detect_range | int | 是 | — | 检测窗口，单位为秒；必须大于 0，且不能小于查询的 `agg_interval` |
...
- 含 NewSeries 的监控项只能配置一个 `query_config`。
...
创建时不传策略 `id`。调用前请替换业务 ID、结果表、指标、聚合维度和通知组 ID。
      ...
      "query_configs": [
        {
          ...
          "data_type_label": "time_series",
          "alias": "a",
          "result_table_id": "system.example",
          "agg_method": "AVG",
          "agg_interval": 60,
          "agg_dimension": [
            "bk_target_ip",
