...
class IncidentMetricsSearchResource(BaseIncidentMetricsResource):
    ...
    def execute_query(self, validated_request_data: dict, query_requests: dict, base_response: dict) -> dict:
        ...
        def _aggregation(req_data: dict[str, Any], metric_name: str, dimension_type: str):
            ...
            with self._lock:
                self.format_metrics_response(unify_query_resp, metric_query_response, metric_name,
    ...
    def build_metrics_query_requests(self, validated_request_data: dict) -> dict:
        ...
        table: str = self.get_tables_by_entity_type(
        ...
        query_requests = self.get_config_by_dimensions(
            dimensions, **config_extra_params)
        return query_requests

    def format_metrics_response(self, unify_query_resp: dict[str, Any], metric_query_response: dict[str, Any], metric_name: str, **kwargs):
        """
        格式化指标查询响应
        """
        # 初始化或获取metric_info
