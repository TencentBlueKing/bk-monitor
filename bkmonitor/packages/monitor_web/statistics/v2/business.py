    ...
    def host_biz_count(self):
        ...
        try:
            records = query.query_data(
                start_time=now_ts.replace(minutes=-3).int_timestamp * 1000, end_time=now_ts.int_timestamp * 1000
            )
        except Exception:
    ...
    def business_active_count(self, metric: Metric):
        ...
        for le_en, seconds in TIME_RANGE:
            ...
            active_business_list = redis_cli.zrevrangebyscore(
                ACTIVE_BIZ_LAST_VISIT_TIME, now.int_timestamp, start_time.int_timestamp
            )
