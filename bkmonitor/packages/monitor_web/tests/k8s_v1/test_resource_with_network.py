...
from bkmonitor.models import BCSIngress, BCSPod, BCSService
...
def _make_namespace_queryset(names, bk_biz_id=2, bcs_cluster_id="BCS-K8S-00000"):
    """创建内存中的 NameSpaceQuerySet，模拟 K8sNamespaceMeta.distinct 返回"""
    return NameSpaceQuerySet(
        [NameSpace({"bk_biz_id": bk_biz_id, "bcs_cluster_id": bcs_cluster_id, "namespace": name}) for name in names]
    )


def _make_ingress_objects(ingress_list, bk_biz_id=2, bcs_cluster_id="BCS-K8S-00000"):
    """创建内存中的 BCSIngress 对象列表"""
    result = []
    for name, namespace in ingress_list:
        obj = BCSIngress()
        obj.name = name
...
def _make_service_objects(service_list, bk_biz_id=2, bcs_cluster_id="BCS-K8S-00000"):
    """创建内存中的 BCSService 对象列表"""
    result = []
    for name, namespace in service_list:
        obj = BCSService()
        obj.name = name
        obj.namespace = namespace
        obj.bk_biz_id = bk_biz_id
...
def _make_pod_objects(pod_list, bk_biz_id=2, bcs_cluster_id="BCS-K8S-00000"):
    """创建内存中的 BCSPod 对象列表
    pod_list: list of (name, namespace, workload_str) or (name, namespace)
    """
    result = []
    for item in pod_list:
        ...
        obj.bcs_cluster_id = bcs_cluster_id
        obj.name = item[0]
        obj.namespace = item[1]
        if len(item) > 2:
            workload = item[2]
            obj.workload_type = workload.split(":")[0]
            obj.workload_name = workload.split(":")[1]
        else:
            obj.workload_type = ""
            obj.workload_name = ""
        result.append(obj)
...
class TestResourceTrendResourceWithNetwork:
    """测试 network 场景下 ResourceTrendResource 的各个 resource_type"""

    @pytest.mark.parametrize(["column"], [pytest.param(c) for c in NETWORK_COLUMNS])
...
class TestListK8SResourcesWithNetwork:
    """测试 network 场景下 ListK8SResources 的各种资源类型和查询模式"""

    # ---- namespace exclude_history ----
    ...
    @apply_setup_filter_mocks
    def test_namespace_exclude_history(self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct):
    ...
    # ---- ingress exclude_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sIngressMeta.distinct")
    @mock.patch("monitor_web.k8s.core.meta.K8sIngressMeta.get_from_meta")
    @apply_setup_filter_mocks
    def test_ingress_exclude_history(self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct):
        mock_ingress_list = [
            ("bcs-ui", "bcs-system"),
        ...
        objs = _make_ingress_objects(mock_ingress_list)
        mock_get_from_meta.return_value = objs
        mock_distinct.return_value = objs
        ...
        expected_items = [{"ingress": ingress, "namespace": namespace} for ingress, namespace in mock_ingress_list]
        result = ListK8SResources()(validated_request_data)
        assert result["items"] == expected_items

    # ---- service exclude_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sServiceMeta.distinct")
    @mock.patch("monitor_web.k8s.core.meta.K8sServiceMeta.get_from_meta")
    @apply_setup_filter_mocks
    def test_service_exclude_history(self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct):
        mock_service_list = [
        ...
        objs = _make_service_objects(mock_service_list)
        mock_get_from_meta.return_value = objs
        mock_distinct.return_value = objs
        ...
        expected_items = [{"service": service, "namespace": namespace} for service, namespace in mock_service_list]
        result = ListK8SResources()(validated_request_data)
        assert result["items"] == expected_items
    ...
    @apply_setup_filter_mocks
    def test_pod_exclude_history(self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct):
        mock_pod_list = [
            (
        ...
        objs = _make_pod_objects(mock_pod_list)
        mock_get_from_meta.return_value = objs
        mock_distinct.return_value = objs
    ...
    # ---- 翻页 namespace exclude_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sNamespaceMeta.distinct")
    @mock.patch("monitor_web.k8s.core.meta.K8sNamespaceMeta.get_from_meta")
    @apply_setup_filter_mocks
    def test_next_page_namespace_exclude_history(
        self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct
    ):
        mock_namespace_list = [
            "aiops-default",
            "apm-demo",
        ...
        ns_qs = _make_namespace_queryset(mock_namespace_list)
        mock_get_from_meta.return_value = ns_qs
        mock_distinct.return_value = ns_qs
    ...
    # ---- 翻页 ingress exclude_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sIngressMeta.distinct")
    @mock.patch("monitor_web.k8s.core.meta.K8sIngressMeta.get_from_meta")
    @apply_setup_filter_mocks
    def test_next_page_ingress_exclude_history(
        self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct
    ):
        mock_ingress_list = [
            ("bcs-ui", "bcs-system"),
    ...
    # ---- 翻页 service exclude_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sServiceMeta.distinct")
    @mock.patch("monitor_web.k8s.core.meta.K8sServiceMeta.get_from_meta")
    @apply_setup_filter_mocks
    def test_next_page_service_exclude_history(
        self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct
    ):
        namespace = "aiops-default"
        mock_service_list = [(f"service-{i}", namespace) for i in range(10)]
        objs = _make_service_objects(mock_service_list)
        mock_get_from_meta.return_value = objs
        mock_distinct.return_value = objs
        ...
        expected_items = [{"service": service, "namespace": ns} for service, ns in mock_service_list]
        result = ListK8SResources()(validated_request_data)
        assert result["items"] == expected_items

    # ---- 翻页 pod exclude_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sPodMeta.distinct")
    @mock.patch("monitor_web.k8s.core.meta.K8sPodMeta.get_from_meta")
    @apply_setup_filter_mocks
    def test_next_page_pod_exclude_history(self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct):
        mock_pod_list = [
            (
                "python-backend--0--session-default---experiment-clear-backbvcgm",
                "aiops-default",
    ...
    # ---- 模糊查询 namespace exclude_history ----
    ...
    def test_namespace_with_query_string_exclude_history(
        ...
        ns_qs = _make_namespace_queryset([namespace_name])
        mock_get_from_meta.return_value = ns_qs
        mock_distinct.return_value = ns_qs
        ...
        expected_items = [{"bk_biz_id": 2, "bcs_cluster_id": "BCS-K8S-00000", "namespace": namespace_name}]
        result = ListK8SResources()(validated_request_data)
        assert result["items"] == expected_items

    # ---- 模糊查询 ingress exclude_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sIngressMeta.distinct")
    @mock.patch("monitor_web.k8s.core.meta.K8sIngressMeta.get_from_meta")
    @apply_setup_filter_mocks
    def test_ingress_with_query_string_exclude_history(
        self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct
    ):
        objs = _make_ingress_objects([("bcs-ui", "bcs-system")])
        mock_get_from_meta.return_value = objs
        mock_distinct.return_value = objs
        ...
        expected_items = [{"ingress": "bcs-ui", "namespace": "bcs-system"}]
        result = ListK8SResources()(validated_request_data)
        assert result["items"] == expected_items

    # ---- 模糊查询 service exclude_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sServiceMeta.distinct")
    @mock.patch("monitor_web.k8s.core.meta.K8sServiceMeta.get_from_meta")
    @apply_setup_filter_mocks
    def test_service_with_query_string_exclude_history(
        self, mock_space_uid, mock_cluster_info, mock_get_from_meta, mock_distinct
    ):
        objs = _make_service_objects([("bcs-ui", "bcs-system")])
        mock_get_from_meta.return_value = objs
        mock_distinct.return_value = objs
        ...
        expected_items = [{"service": "bcs-ui", "namespace": "bcs-system"}]
        result = ListK8SResources()(validated_request_data)
        assert result["items"] == expected_items

    # ---- 模糊查询 pod exclude_history ----
    ...
    def test_pod_with_query_string_exclude_history(
        ...
        pod_data = [("bcs-ui-687c796845-452tc", "bcs-system", "Deployment:bcs-ui")]
        objs = _make_pod_objects(pod_data)
        mock_get_from_meta.return_value = objs
        mock_distinct.return_value = objs
        ...
        expected_items = [
            {"pod": "bcs-ui-687c796845-452tc", "namespace": "bcs-system", "workload": "Deployment:bcs-ui"}
        ]
        result = ListK8SResources()(validated_request_data)
        assert result["items"] == expected_items

    # ---- namespace with_history ----
    ...
    def test_namespace_with_history(self, mock_space_uid, mock_cluster_info, graph_unify_query, mock_get_from_meta):
        ...
        mock_get_from_meta.return_value = []
    ...
    def test_namespace_with_history_and_filter(
        ...
        expected_items = [{"bk_biz_id": 2, "bcs_cluster_id": "BCS-K8S-00000", "namespace": "kube-system"}]
    ...
    # ---- ingress with_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sIngressMeta.get_from_meta")
    @mock.patch("core.drf_resource.resource.grafana.graph_unify_query")
    @apply_setup_filter_mocks
    def test_ingress_with_history(self, mock_space_uid, mock_cluster_info, graph_unify_query, mock_get_from_meta):
        mock_ingress_list = [
    ...
    # ---- ingress with_history + filter ----

    @mock.patch("monitor_web.k8s.core.meta.K8sIngressMeta.get_from_meta")
    @mock.patch("core.drf_resource.resource.grafana.graph_unify_query")
    @apply_setup_filter_mocks
    def test_ingress_with_history_and_filter(
        self, mock_space_uid, mock_cluster_info, graph_unify_query, mock_get_from_meta
    ):
        namespace = "blueking"
        mock_ingress_list = [
        ...
        expected_items = [{"ingress": ingress, "namespace": namespace} for ingress in mock_ingress_list]
    ...
    # ---- service with_history ----

    @mock.patch("monitor_web.k8s.core.meta.K8sServiceMeta.get_from_meta")
    @mock.patch("core.drf_resource.resource.grafana.graph_unify_query")
    @apply_setup_filter_mocks
    def test_service_with_history(self, mock_space_uid, mock_cluster_info, graph_unify_query, mock_get_from_meta):
        mock_service_list = [
    ...
    # ---- service with_history + filter ----

    @mock.patch("monitor_web.k8s.core.meta.K8sServiceMeta.get_from_meta")
    @mock.patch("core.drf_resource.resource.grafana.graph_unify_query")
    @apply_setup_filter_mocks
    def test_service_with_history_and_filter(
        self, mock_space_uid, mock_cluster_info, graph_unify_query, mock_get_from_meta
    ):
        namespace = "blueking"
        mock_service_list = [
        ...
        expected_items = [{"service": service, "namespace": namespace} for service in mock_service_list]
    ...
    @mock.patch("core.drf_resource.resource.grafana.graph_unify_query")
    @apply_setup_filter_mocks
    def test_pod_with_with_history(self, mock_space_uid, mock_cluster_info, graph_unify_query, mock_get_from_meta):
        mock_pod_list = [
        ...
        expected_items = [{"pod": pod, "namespace": namespace} for pod, namespace in mock_pod_list]
    ...
    def test_pod_with_history_and_filter(
        ...
        expected_items = [{"pod": pod, "namespace": namespace} for pod in mock_pod_list]
    ...
    # ---- namespace with_history（PromQL 不足从 DB 补充） ----

    @mock.patch("monitor_web.k8s.core.meta.K8sNamespaceMeta.get_from_meta")
    @mock.patch("core.drf_resource.resource.grafana.graph_unify_query")
    @apply_setup_filter_mocks
    def test_namespace_with_history_1(self, mock_space_uid, mock_cluster_info, graph_unify_query, mock_get_from_meta):
        """
        覆盖 promql 查询不足，还需要从 db 中补充的场景，以及需要进行去重操作
        """
        mock_namespace_list = [
            "bkbase",
            "blueking",
        ...
        # PromQL 只返回前5个，DB补充后5个（其中 kube-system 重复）
        promql_namespaces = mock_namespace_list[:5]
        db_namespaces = mock_namespace_list[4:]  # kube-system 开始

        # mock get_from_meta 返回 DB 补充数据（NameSpace 字典列表）
        mock_get_from_meta.return_value = [
            NameSpace({"bk_biz_id": 2, "bcs_cluster_id": "BCS-K8S-00000", "namespace": ns}) for ns in db_namespaces
        ]
        ...
        graph_unify_query.return_value = {
            "series": [
                ...
                for namespace in promql_namespaces
            ],
            "metrics": [],
        }
