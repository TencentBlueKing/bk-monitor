...
class TestPromQL:
    ...
    NETWORK_ARGVALUES = [
        ...
        (
            ...
            {},
            "",
            "topk(10, sum by (ingress, namespace) ((count by (bk_biz_id, bcs_cluster_id, namespace, ingress, service, pod)\n            (ingress_with_service_relation{bcs_cluster_id=\"BCS-K8S-00000\",bk_biz_id=\"2\"}) * 0 + 1)\n            * on (namespace, service) group_left(pod)\n            (count by (service, namespace, pod) (pod_with_service_relation))\n            * on (namespace, pod) group_left()\n            sum by (namespace, pod)\n            (last_over_time(\n            rate(container_network_receive_bytes_total{}[1m])[1m:]))) * -1) * -1",  # noqa
        ),
        ...
        (
            ...
            {},
            "",
            "topk(10, sum by (namespace, service) ((count by (service, namespace, pod) (pod_with_service_relation{bcs_cluster_id=\"BCS-K8S-00000\",bk_biz_id=\"2\"}) * 0 + 1) * on (namespace, pod) group_left()\n            sum by (namespace, pod)\n            (last_over_time(\n            rate(container_network_receive_bytes_total{}[1m])[1m:]))) * -1) * -1",  # noqa
        ),
        # pod
        # - nothing
        (
            ...
            {},
            "",
            "topk(10, label_replace(sum by (namespace, pod) (\n            sum by (namespace, pod)\n            (last_over_time(rate(container_network_receive_bytes_total{}[1m])[1m:]))),\n            \"pod_name\", \"$1\", \"pod\", \"(.*)\") * -1) * -1",  # noqa
        ),
    ]
    CAPACITY_ARGVALUES = []
    ...
    @pytest.mark.parametrize(
        ...
        [pytest.param(*item, id=" | ".join([*item[:2], str(item[2]), item[3]])) for item in PERFORMANCE_ARGVALUES],
    )
    ...
    @pytest.mark.parametrize(
        ["resource_type", "column", "filter_dict", "query_string", "expected_promql"],
        [pytest.param(*item, id=" | ".join([*item[:2], str(item[2]), item[3]])) for item in NETWORK_ARGVALUES],
    )
