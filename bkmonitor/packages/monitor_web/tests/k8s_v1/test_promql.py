    ...
    NETWORK_ARGVALUES = [
        ...
        (
            ...
            topk(
              10,
              sum by (namespace) (
                sum by (namespace, pod) (last_over_time(rate(container_network_receive_bytes_total{bcs_cluster_id="BCS-K8S-00000"}[1m])[1m:]))
              ) * -1
            ) * -1
            """,
        ),
        ...
        (
            ...
            topk(
              ...
              sum by (namespace, service) (
                ...
                * on (namespace, pod) group_left ()
                  sum by (namespace, pod) (
                    last_over_time(rate(container_network_receive_bytes_total{bcs_cluster_id="BCS-K8S-00000",namespace="aiops-default"}[1m])[1m:])
                  )
              ) * -1
            ) * -1
            """,
        ...
        (
            ...
            topk(
              ...
              label_replace(
                sum by (namespace, pod) (
                  sum by (namespace, pod) (
                    last_over_time(rate(container_network_receive_bytes_total{bcs_cluster_id="BCS-K8S-00000"}[1m])[1m:])
                  )
                ),
                "pod_name",
                "$1",
