...
def cache_application_scope_name():
    ...
    for application in Application.objects.filter(is_enabled=True):
        try:
            ...
            service_nodes = ServiceHandler.list_nodes(bk_biz_id, application_name)
            collection = {}
            for service_node in service_nodes:
                service_name = service_node["topo_key"]
                monitor_info_mapping = MetricHelper.get_monitor_info(bk_biz_id, result_table_id, service_name)
                collection[service_name] = monitor_info_mapping
            cache_key = ApmCacheKey.APP_SCOPE_NAME_KEY.format(bk_biz_id=bk_biz_id, application_id=application_id)
            cache_agent.set(cache_key, json.dumps(collection))
