...
def build_event_body(app: Application, bk_biz_id: int, apm_event: APMEvent):
...
@task(ignore_result=True)
def report_apm_application_event(bk_biz_id, application_id, apm_event=APMEvent.APP_CREATE):
    logger.info(f"[report_apm_application_event] task start, bk_biz_id({bk_biz_id}), application_id({application_id})")

    application = Application.objects.get(application_id=application_id)
    data = build_event_body(application, bk_biz_id, apm_event)
