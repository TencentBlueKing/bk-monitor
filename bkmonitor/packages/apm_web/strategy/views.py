...
class StrategyTemplateViewSet(GenericViewSet):
    ...
    def batch_partial_update(self, *args, **kwargs) -> Response:
        ...
        model_serializer.validate_is_auto_apply(strategy_template_objs, validated_data)
        for obj in strategy_template_objs:
            model_serializer.set_auto_apply(validated_data, obj, validated_data["update_time"])

            # 设置更新字段。
            for field_name in validated_data:
                setattr(obj, field_name, validated_data[field_name])
            validated_data.pop("auto_applied_at", None)
