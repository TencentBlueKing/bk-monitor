...
def _mock_push_url_dependencies(mocker, proxy_hosts=None):
    ...
    api = mocker.Mock()
    mocker.patch("apm_web.meta.resources.api", new=api)
    api.node_man.get_proxies_by_biz.return_value = proxy_hosts or []
...
def test_push_url_uses_proxy_ip_when_central_domain_is_empty(mocker):
    ...
    with override_settings(
        ...
        CUSTOM_REPORT_DEFAULT_PROXY_IP=["192.0.2.1"],
    ...
    assert [item["push_url"] for item in endpoints] == ["http://192.0.2.1", "http://cluster.example"]
