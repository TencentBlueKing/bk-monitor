...
class ServiceLogTaskHandler:
    ...
    def get_k8s_related_log_indexes(cls, bk_biz_id: int, app_name: str) -> dict[str, list[dict[str, Any]]]:
        ...
        with ThreadPool(2) as pool:
            ...
            workload_indexes: dict[WorkloadKey, list[dict[str, Any]]] = workload_future.get()
            service_indexes: dict[str, list[dict[str, Any]]] = service_future.get()
    ...
    def _query_workload_related_indexes(
        ...
        def _query_chunk(workload_chunk: list[dict[str, str]]) -> dict[WorkloadKey, set[int | str]]:
            ...
            for relation in RelationQ.query(relation_qs, fill_with_empty=True):
                if not relation:
                    continue
            ...
            return dict(related_data_ids)

        related_data_ids: dict[WorkloadKey, set[int | str]] = {}
        with ThreadPool(min(len(workload_chunks), cls.QUERY_CONCURRENCY)) as pool:
            for chunk_result in pool.imap_unordered(_query_chunk, workload_chunks):
                related_data_ids.update(chunk_result)

        return cls._convert_data_ids_to_indexes(bk_biz_id, related_data_ids)
    ...
    def _query_service_related_indexes(
        ...
        def _query_chunk(service_name_chunk: list[str]) -> dict[str, set[int | str]]:
            source_infos: list[Source] = [
        ...
        with ThreadPool(min(len(service_name_chunks), cls.QUERY_CONCURRENCY)) as pool:
            for chunk_result in pool.imap_unordered(_query_chunk, service_name_chunks):
                related_data_ids.update(chunk_result)
