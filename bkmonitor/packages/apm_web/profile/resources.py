...
class QueryServicesDetailResource(Resource):
    ...
    class RequestSerializer(serializers.Serializer):
        ...
        profile_type = serializers.CharField(required=True, label="数据类型")

    def perform_request(self, validated_data):
        services = api.apm_api.query_profile_services_detail(**validated_data)
        ...
        info = QueryTemplate(validated_data["bk_biz_id"], validated_data["app_name"]).get_sample_info(
            validated_data["start_time"], validated_data["end_time"], validated_data["profile_type"]
        )
        ...
        return {
            ...
            "create_time": services[-1].get("created_at"),
