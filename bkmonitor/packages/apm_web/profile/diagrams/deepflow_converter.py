...
class DeepFlowConverter(TreeConverter):
    ...
    def convert(self, raw: list, data_type: str):
        ...
        if raw[0]["parent_node_id"] != "-1":
            raise ValueError(
                "Invalid data format: the first result of deepflow must be the root node with parent_node_id '-1'."
            )
        ...
        # 构建树/图结构 value 已经由 server 计算过 父子关系由 parent id 和 node id 可直接获得 server 已经计算过
        for result in raw:
            ...
            if tree.function_node_map.get(parent_id, None):
                ...
                tree.map_root.add_child(node)
        self.tree = tree
        self.tree.map_root = self.tree.root
