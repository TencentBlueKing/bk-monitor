...
class CompatibleIAM(IAM):
    ...
    def _merge_alias_policies(self, data, policies):
        """
        ...
        """
        for alias_action_id in ACTION_COMPATIBLE_ALIASES.get(data["action"]["id"], []):
            alias_data = copy.deepcopy(data)
            alias_data["action"]["id"] = alias_action_id
            _, _, alias_policies = self._client.policy_query(alias_data)
    ...
    def _do_policy_query(self, request, with_resources=True):
        ...
        if not self.in_compatibility_mode():
            ...
            return self._merge_alias_policies(data, policies)
        ...
        policies = self._merge_alias_policies(data, policies)
