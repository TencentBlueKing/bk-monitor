...
def get_plan_id_by_algorithm(algorithm_id):
    ...
    result_plan_id = getattr(settings, algorithm_info.env_plan_variate_name)

    # 如果方案配置已经迁移，且还需要走bkbase的方式接入，则需要从迁移后的配置中拿到原来的方案ID
    if settings.IS_MIGRATE_AIOPS_STRATEGY:
        algorithm_choice = AlgorithmChoiceConfig.objects.filter(id=result_plan_id).first()
        if algorithm_choice and algorithm_choice.config.get("bkbase_plan_id"):
            result_plan_id = algorithm_choice.config["bkbase_plan_id"]
