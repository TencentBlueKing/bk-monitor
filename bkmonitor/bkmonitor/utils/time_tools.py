...
def get_user_timezone(username: str) -> pytz.tzinfo.BaseTzInfo | None:
    ...
    try:
        ...
        user_info = api.bk_login.get_user_info(id=username, fields="time_zone")
        user_time_zone = user_info.get("time_zone", "")

        if not user_time_zone:
            logger.info(f"Get user timezone success but empty: user={username}, fallback to biz_tz")
            return None
        ...
        logger.warning(f"Get user timezone failed: user={username}, error={e}, fallback to biz_tz")
        return None
