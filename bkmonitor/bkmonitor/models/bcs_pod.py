...
class BCSPod(BCSBase, BCSBaseResources):
    ...
    def update_usage_resource_and_monitor_status(
        ...
        existing_hash_to_instance = {pod.unique_hash: pod for pod in pods}
        unique_hash_set_for_update = set(existing_hash_to_instance) & set(hash_to_usage)
        pods_for_update = []
        for unique_hash in unique_hash_set_for_update:
            new_usage = hash_to_usage[unique_hash]
            pod = existing_hash_to_instance[unique_hash]

            pod.resource_usage_cpu, pod.resource_usage_memory, pod.resource_usage_disk, pod.monitor_status = new_usage
            pod.request_cpu_usage_ratio = (
                round(force_float(pod.resource_usage_cpu / pod.resource_requests_cpu), percent_precision)
                if pod.resource_usage_cpu and pod.resource_requests_cpu
                else 0
            ) * 100
            pod.limit_cpu_usage_ratio = (
                round(force_float(pod.resource_usage_cpu / pod.resource_limits_cpu), percent_precision)
                if pod.resource_usage_cpu and pod.resource_limits_cpu
                else 0
            ) * 100
            pod.request_memory_usage_ratio = (
                round(force_float(pod.resource_usage_memory / pod.resource_requests_memory), percent_precision)
                if pod.resource_usage_memory and pod.resource_requests_memory
                else 0
            ) * 100
            pod.limit_memory_usage_ratio = (
                round(force_float(pod.resource_usage_memory / pod.resource_limits_memory), percent_precision)
                if pod.resource_usage_memory and pod.resource_limits_memory
                else 0
            ) * 100
        ...
        # 将未采集的记录设置为初始状态
        unique_hash_set_for_reset = set(existing_hash_to_instance) - set(hash_to_usage)
    ...
    @classmethod
    def sync_resource_usage(cls, bk_biz_id: int) -> None:
        """同步指定集群的资源使用量和数据状态 ."""
        # 获得cpu, memory, disk使用量
        group_by = ["bcs_cluster_id", "namespace", "pod_name"]
        usages = cls.fetch_container_usage(bk_biz_id, group_by)
        # 获得资源的数据状态
