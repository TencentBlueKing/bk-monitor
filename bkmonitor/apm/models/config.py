...
class PlatformConfig(models.Model):
    ...
    REFRESH_CONFIG_KEYS = ["config_data"]
    UNIQUE_KEYS = ["config_type"]

    config_type = models.CharField("配置类型", max_length=50)
    config_data = JsonField(verbose_name="配置数据")
    ...
    def get_config(cls, config_type):
        """获取指定类型的配置"""
        # 查找全局配置
        config = cls.objects.filter(config_type=config_type).first()
    ...
    @classmethod
    def init_builtin_config_with_data(cls, config_type, config_data):
        """使用指定配置数据初始化配置"""
        if not config_data:
            raise ValueError("配置数据不能为空")

        # 查找现有配置
        existing_config = cls.objects.filter(config_type=config_type).first()

        if existing_config:
            # 合并更新现有配置
            if isinstance(existing_config.config_data, dict) and isinstance(config_data, dict):
                # 如果都是字典类型，进行合并
                existing_config.config_data.update(config_data)
            else:
                # 否则直接替换
                existing_config.config_data = config_data
            existing_config.save()
            logger.info(f"[PlatformConfig] update builtin config: {config_type}")
        else:
            # 创建新配置
            config_obj = cls(
            ...
            config_obj.save()
            logger.info(f"[PlatformConfig] create builtin config: {config_type}")
