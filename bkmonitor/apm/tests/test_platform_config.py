...
def test_get_target_host_ids_by_tenant_excludes_blacklisted_biz(settings, mocker):
    ...
    host_ids = BkCollectorConfig.get_target_host_ids_by_bk_tenant_id("tenant-1", exclude_blacklisted_biz=True)

    assert host_ids == [101]
    get_proxies.assert_called_once_with(bk_tenant_id="tenant-1", bk_cloud_id=1)


def test_get_target_host_ids_by_tenant_keeps_blacklisted_biz_by_default(settings, mocker):
    settings.NEW_ENV_BIZ_BLACK_LIST = [12]
    mocker.patch(
        "bkmonitor.utils.bk_collector_config.api.cmdb.search_cloud_area",
        return_value=[{"bk_cloud_id": 1}],
    )
    mocker.patch(
        "bkmonitor.utils.bk_collector_config.api.node_man.get_proxies",
        return_value=[{"bk_host_id": 102, "bk_biz_id": 12, "status": "RUNNING"}],
    )
...
def test_refresh_excludes_blacklisted_biz_and_keeps_default_hosts(mocker):
    ...
    get_target_hosts.assert_called_once_with(DEFAULT_TENANT_ID, exclude_blacklisted_biz=True)
