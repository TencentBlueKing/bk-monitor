...
test('事件指标名提示使用换行纯文本', () => {
  ...
  assert.match(source, /content:\s*data\.map\(item => item\.name\)\.join\('\\n'\)/);
  assert.match(source, /\$bkPopover\(e\.target,\s*\{[^}]*allowHTML:\s*false[^}]*\}\)/s);
  assert.doesNotMatch(source, /content:\s*`<div>\$\{item\.name\}<\/div>`/);
});
...
test('事故名称与标签提示使用纯文本', () => {
  ...
  assert.match(source, /content:\s*data\.map\(item => \(typeof item === 'string' \? item : `\$\{item\.key\}：\$\{item\.value\}`\)\)\.join\('\\n'\)/);
  assert.match(source, /\$bkPopover\(e\.target,\s*\{[^}]*allowHTML:\s*false[^}]*\}\)/s);
