...
class AppServiceQueryResource(Resource):
    ...
    def batch_query_deepflow_service(
        cls, deep_flow_server_clusters: list, deep_flow_server_clusters_mapping: dict, params: dict
    ):
        def inner(base_url, ebpf_param):
        ...
        # 遍历集群信息列表
        for cluster_info in deep_flow_server_clusters:
            cluster_id = cluster_info.get('clusterID')
            # 获取对应的 server 地址
            base_url = deep_flow_server_clusters_mapping.get(cluster_id)
            if base_url:
        ...
        for cluster_id, future in futures.items():
            try:
                ...
                for service in services_list:
                    service["cluster"] = cluster_id
                    serivce_data.append(service)
            except Exception as e:
                logger.info(f"batch_query_deepflow, cluster ID {cluster_id}, {e}")
        return serivce_data
    ...
    def perform_request(self, params):
        ...
        return self.batch_query_deepflow_service(deep_flow_server_clusters, deep_flow_server_clusters_mapping, params)
