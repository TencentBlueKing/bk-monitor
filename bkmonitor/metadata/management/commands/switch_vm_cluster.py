...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        SpaceTableIDRedis().push_table_id_detail(
            table_id_list=[obj.result_table_id for obj in objs], is_publish=True, bk_tenant_id=bk_tenant_id
        )
        self.stdout.write(self.style.SUCCESS(f"switch vm cluster success, {updated_count} records updated."))

    def add_arguments(self, parser):
        parser.add_argument("--bk_tenant_id", required=True, help="要切换的资源对应的租户id")
    ...
    def validate(self, **options):
        ...
        if not bk_tenant_id:
            raise CommandError("bk_tenant_id is required")
        ...
        dst_vm_cluster_id = self.get_dst_vm_cluster_id(
            bk_tenant_id=bk_tenant_id, dst_vm_id=dst_vm_id, dst_vm_name=dst_vm_name
        )
        # 过滤对应的 vm 记录
        objs = self.filter_vm_records(
            bk_tenant_id=bk_tenant_id, src_vm_names=src_vm_names, src_vm_ids=src_vm_ids, data_ids=data_ids
        )
        if not objs:
    ...
    def filter_vm_records(
        self,
        bk_tenant_id: str,
        src_vm_names: str | None = None,
        src_vm_ids: str | None = None,
        ...
        if data_ids:
            ...
            table_id_list = list(
                models.DataSourceResultTable.objects.filter(
                    bk_data_id__in=data_ids, bk_tenant_id=bk_tenant_id
                ).values_list("table_id", flat=True)
            )
            return models.AccessVMRecord.objects.filter(result_table_id__in=table_id_list, bk_tenant_id=bk_tenant_id)

        if src_vm_ids:
            src_vm_ids = [int(id) for id in src_vm_ids.split(",")]
            return models.AccessVMRecord.objects.filter(vm_cluster_id__in=src_vm_ids, bk_tenant_id=bk_tenant_id)

        src_vm_names = src_vm_names.split(",")
        cluster_id_list = models.ClusterInfo.objects.filter(
            cluster_name__in=src_vm_names, bk_tenant_id=bk_tenant_id
        ).values_list("cluster_id", flat=True)
        return models.AccessVMRecord.objects.filter(vm_cluster_id__in=cluster_id_list, bk_tenant_id=bk_tenant_id)

    def get_dst_vm_cluster_id(
        self, bk_tenant_id: str, dst_vm_id: str | None = None, dst_vm_name: str | None = None
    ) -> int:
        """过滤要切换的vm集群ID"""
        filter_params = Q()
        if dst_vm_id:
        ...
        obj = models.ClusterInfo.objects.filter(filter_params, bk_tenant_id=bk_tenant_id).first()
        if not obj:
