...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for tenant in api.bk_login.list_tenant():
            try:
                biz_list = api.cmdb.get_business(bk_tenant_id=tenant["id"])
                create_bkcc_spaces(
                    [
                        {"bk_biz_id": str(biz.bk_biz_id), "bk_biz_name": biz.bk_biz_name, "bk_tenant_id": tenant["id"]}
                        for biz in biz_list
                    ]
                )
            except BKAPIError as e:
                print(f"[sync_cmdb_space] sync bkcc space failed, bk_tenant_id: {tenant['id']}, error: {e}")
        # 赋值空间级的数据源 ID
