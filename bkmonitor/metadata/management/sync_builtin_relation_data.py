    ...
    def handle(self, *args, **options):
        ...
        for field, value in redis_data.items():
            ...
            if rt:
                if not token:
                    try:
                        self.stdout.write("Field {} RT exist but token is empty,start generate token".format(key))
                        ...
                        self.stdout.write("Generate Token For Field {} has completed".format(key))
                    except Exception as e:
                        self.stdout.write(f"Error: Failed to write data to redis, error={e}, field={key}")
            else:
                if not token:
                    try:
                        self.stdout.write("Field {} RT not exist and token is empty,start generate new one".format(key))
                        ...
                        self.stdout.write(f"Error: Failed to create new DS&RT, error={e}, field={key}")
                        continue
