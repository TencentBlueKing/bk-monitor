...
def migrate_custom_ts_field_to_time_series(apps, schema_editor):
    ...
    logger.info(f"[数据加载] CustomTSField 加载完成: {len(all_custom_fields)} 条, 耗时 {load_data_cost:.2f}s")
    ...
    logger.info(f"[数据加载] CustomTSGroupingRule 加载完成: {len(all_grouping_rules)} 条, 耗时 {load_rules_cost:.2f}s")
    ...
    summary = (
        ...
        f"耗时: 数据加载 {load_data_cost + load_rules_cost:.2f}s, 数据处理 {process_cost:.2f}s, 总计 {total_cost:.2f}s"
    )
