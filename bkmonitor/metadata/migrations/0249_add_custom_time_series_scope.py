...
def migrate_custom_ts_field_to_time_series(apps, schema_editor):
    ...
    # 分批处理 groups，避免一次性加载所有 CustomTSField 导致内存溢出
    for i in range(0, stats["total_groups"], BATCH_SIZE):
        batch_start = i
        batch_end = min(i + BATCH_SIZE, stats["total_groups"])
        batch_tables = list(ts_tables[batch_start:batch_end])
        logger.info(f"[批次处理] 处理 {batch_start + 1}-{batch_end}/{stats['total_groups']} 个分组")

        # 收集该批次的所有 group_id
        batch_group_ids = [ts_table["time_series_group_id"] for ts_table in batch_tables]

        # 仅加载该批次的 CustomTSField 和 CustomTSGroupingRule
        load_data_start = time.time()
        all_custom_fields = list(
            CustomTSField.objects.filter(time_series_group_id__in=batch_group_ids).order_by(
        ...
        load_data_cost = time.time() - load_data_start

        load_rules_start = time.time()
        all_grouping_rules = list(
            CustomTSGroupingRule.objects.filter(time_series_group_id__in=batch_group_ids).order_by(
                "time_series_group_id", "id"
            )
        )
        ...
        load_rules_cost = time.time() - load_rules_start

        logger.info(
            f"[批次数据加载] 加载完成: CustomTSField {len(all_custom_fields)} 条, "
            f"CustomTSGroupingRule {len(all_grouping_rules)} 条, "
            f"耗时 {load_data_cost + load_rules_cost:.2f}s"
        )

        # 处理该批次的每个 group
        for ts_table in batch_tables:
        ...
        del batch_tables
        del batch_group_ids
        del all_custom_fields
        del all_grouping_rules
        del fields_by_group
        del grouping_rules_by_group
        gc.collect()
        logger.info(f"[批次完成] 批次 {batch_start + 1}-{batch_end} 内存已释放")
    ...
    summary = (
        ...
        f"耗时: 数据处理 {process_cost:.2f}s, 总计 {total_cost:.2f}s"
    )
