...
class ResultTable(models.Model):
    ...
    @classmethod
    def get_table_id_cutter(cls, table_ids: Union[List, Set], bk_tenant_id=DEFAULT_TENANT_ID) -> Dict:
        """获取结果表是否禁用切分模块"""
    ...
    def upgrade_result_table(self, operator):
        ...
        logger.info(
            "result_table->[{}] of bk_tenant_id->[{}] now is marked deleted.".format(self.table_id, self.bk_tenant_id)
        )
    ...
    def get_tag_values(self, tag_name):
        for real_storage in self.real_storage_list:
            if hasattr(real_storage, "get_tag_values"):
                return real_storage.get_tag_values(tag_name)  # 这里无需补充租户ID作为查询条件,因为real_storage已经进行了过滤
...
class ResultTableField(models.Model):
    ...
    def make_cmdb_default_fields(cls, table_id, bk_tenant_id=DEFAULT_TENANT_ID):
        ...
        logger.info(
            "table->[{}] of bk_tenant_id->[{}] now has created default cmdb field.".format(table_id, bk_tenant_id)
        )
    ...
    def _compose_data(
        self, table_id: str, field_data: List[Dict[str, Any]], bk_tenant_id: Optional[str] = DEFAULT_TENANT_ID
    ...
    def batch_get_fields(
        ...
        is_consul_config: Optional[bool] = False,
        bk_tenant_id: Optional[str] = DEFAULT_TENANT_ID,
    ) -> Dict:
...
class ResultTableOption(OptionBase):
    ...
    def create_option(cls, table_id, name, value, creator, bk_tenant_id=DEFAULT_TENANT_ID):
        ...
        logger.info(
            "table_id->[{}] of bk_tenant_id->[{}] now has create option->[{}]".format(table_id, bk_tenant_id, name)
        )
