...
class ResultTable(models.Model):
    ...
    def apply_datalink(self, force_update: bool = False, delay: bool = True) -> None:
        ...
        if self.default_storage == ClusterInfo.TYPE_INFLUXDB:
            ...
            # 如果force_update为True或是开启插件V4链路，则强制将数据链路更新为V4链路
            consumer_group = None
            if force_update or options.get(ResultTableOption.OPTION_ENABLE_PLUGIN_V4_DATA_LINK, False):
            ...
            if (settings.ENABLE_V2_VM_DATA_LINK or not settings.ENABLE_INFLUXDB_STORAGE) and (
                is_v4_datalink_etl_config or not settings.ENABLE_INFLUXDB_STORAGE
            ):
