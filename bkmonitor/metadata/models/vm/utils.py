...
def check_and_create_proxy_vm_data_link(cluster):
    """
    检查该联邦代理集群是否已经创建并存在对应的V4链路并正确配置
    ...
    datalink_name = utils.get_bkdata_data_id_name(ds.data_name)  # 链路标识符
    ...
    create_vm_data_link(
        ...
        data_source=ds,
        vm_cluster_name=settings.BCS_PROXY_CLUSTER_VM_STORAGE_CLUSTER_NAME,
        bcs_cluster_id=cluster.cluster_id,
    )
