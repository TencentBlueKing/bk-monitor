...
class GraphDataBusConfigManager(models.Manager):
    """Limit proxy queries to Databus records generated for graph ResultTables."""

    def get_queryset(self):
        return super().get_queryset().filter(name__contains="_graph")
...
class GraphDataBusConfig(DataBusConfig):
    ...
    注意（查询隔离）：
      ...
      图链路读写入口必须使用 GraphDataBusConfig.objects，该 manager 按图 RT 命名约定隔离。
    """
