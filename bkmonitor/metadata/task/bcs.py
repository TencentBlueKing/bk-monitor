...
from metadata.models.vm.utils import create_fed_bkbase_data_link, get_vm_cluster_id_name
...
def sync_federation_clusters(fed_clusters):
    ...
    logger.info("sync_federation_clusters started.")
    try:
        # 获取传入数据中的所有联邦集群 ID
        fed_cluster_ids = set(fed_clusters.keys())

        # 获取数据库中现有的联邦集群 ID
        existing_fed_clusters = set(models.BcsFederalClusterInfo.objects.values_list('fed_cluster_id', flat=True))

        # 删除不再归属的联邦集群记录
        clusters_to_delete = existing_fed_clusters - fed_cluster_ids
        if clusters_to_delete:
            logger.info("Deleting federation cluster info for->[%s]", clusters_to_delete)
            models.BcsFederalClusterInfo.objects.filter(fed_cluster_id__in=clusters_to_delete).delete()

        # 遍历最新的联邦集群关系
        for fed_cluster_id, fed_cluster_data in fed_clusters.items():
            logger.info("Syncing federation cluster ->[%s]", fed_cluster_id)
            ...
            for sub_cluster_id, namespaces in sub_clusters.items():
                logger.info("Syncing sub-cluster -> [%s]", sub_cluster_id)

                # 获取现有的命名空间记录（当前数据库中已存在的子集群记录）
                existing_records = models.BcsFederalClusterInfo.objects.filter(
                    fed_cluster_id=fed_cluster_id, sub_cluster_id=sub_cluster_id
                )
                ...
                if updated_namespaces == current_namespaces:
                    logger.info(
                        "Sub-cluster->[%s] in federation->[%s] is already up-to-date, skipping",
                        sub_cluster_id,
                        fed_cluster_id,
                    )
                    continue
                ...
                # 如果命名空间有变更，更新记录
                logger.info(
                    "Updating namespaces for sub-cluster->[%s],in federation->[%s]", sub_cluster_id, fed_cluster_id
                )
                ...
                try:
                    ...
                    vm_cluster = get_vm_cluster_id_name(
                        space_type=SpaceTypes.BKCC.value, space_id=str(cluster.bk_biz_id)
                    )
                    ...
                    continue
                logger.info(
                    "Updated federation cluster info for sub-cluster->[%s] in fed->[%s]", sub_cluster_id, fed_cluster_id
                )
        ...
        # 获取数据库中所有子集群的记录
        existing_sub_clusters = models.BcsFederalClusterInfo.objects.values_list('fed_cluster_id', 'sub_cluster_id')

        # 找出不再归属任何联邦集群的子集群记录
        sub_clusters_to_delete = set(existing_sub_clusters) - all_sub_clusters_in_fed_clusters

        if sub_clusters_to_delete:
            logger.info("Deleting sub-clusters that are no longer part of any federation->[%s]", sub_clusters_to_delete)
            # 使用动态条件生成过滤器来删除记录
        ...
        logger.info("sync_federation_clusters finished successfully.")

    except Exception as e:  # pylint: disable=broad-except
        logger.error("sync_federation_clusters failed, error->[%s]", e)
