...
class QueryTimeSeriesScopeMetricsResource(Resource):
    """
    查询分组下的指标

    根据分组的 manual_list 和 auto_rules 规则，查询匹配的指标列表
    如果不传递 scope_name，则返回该分组下的全部指标
    """
    ...
    def perform_request(self, validated_request_data):
        ...
        available_metrics = models.TimeSeriesMetric.objects.filter(group_id=group_id)

        # 如果传递了 scope_name，则根据指标分组规则过滤
        if scope_name:
            # 查询指标分组
            time_series_scope = models.TimeSeriesScope.objects.filter(group_id=group_id, scope_name=scope_name).first()
            if not time_series_scope:
            ...
            # 使用 scope 的 match_metric 方法过滤匹配的指标
            matched_metrics = []
            for metric in available_metrics:
                if time_series_scope.match_metric(metric.field_name):
                    matched_metrics.append(metric)
        else:
            # 如果不传递 scope_name，返回全部指标
            matched_metrics = list(available_metrics)
        ...
        results = [
            {
                ...
                "last_modify_time": metric.last_modify_time.strftime("%Y-%m-%d %H:%M:%S"),
                "label": metric.label,
            }
        ...
        if page_size > 0:
            ...
            return {"count": count, "info": paginated_results}
