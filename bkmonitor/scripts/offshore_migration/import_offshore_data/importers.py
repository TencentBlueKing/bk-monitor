...
class UserGroupImporter(BaseImporter, RelationImportMixin):
    ...
    def import_relations(self, obj, relations: dict):
        ...
        if "duty_arranges" in relations:
            self.import_related_queryset(obj, "duty_arranges", relations["duty_arranges"], DutyArrange, "user_group_id")
...
class AlertAssignImporter(BaseImporter, RelationImportMixin):
    ...
    def import_relations(self, obj, relations: dict):
        ...
        if "rules" in relations:
            self.import_related_queryset(obj, "rules", relations["rules"], AlertAssignRule, "assign_group_id")
...
class DashboardImporter(BaseImporter):
    ...
    def _check_conflict(self, data: dict):
        ...
        if title and org_id is not None:
            try:
                return self.model_class.objects.get(org_id=org_id, folder_id=folder_id, title=title)
...
class CollectConfigImporter(BaseImporter, RelationImportMixin):
    ...
    def _import_plugin_version(self, plugin_version_data: dict, collect_config_id) -> int:
        ...
        """
        from monitor_web.models.plugin import PluginVersionHistory, CollectorPluginConfig, CollectorPluginInfo
        ...
        try:
            existing_version = PluginVersionHistory.objects.get(
                plugin_id=plugin_id, bk_tenant_id=bk_tenant_id, config_version=config_version, info_version=info_version
            )
            logger.info(
                f"PluginVersionHistory for plugin {plugin_id} v{config_version}.{info_version} already exists (id={existing_version.id})"
            )
        ...
        if config_data:
            ...
            try:
                ...
                logger.error(
                    f"Failed to create CollectorPluginConfig for collect_config {collect_config_id}: {e}", exc_info=True
                )
                raise
        ...
        if info_data:
            ...
            try:
                ...
                logger.error(
                    f"Failed to create CollectorPluginInfo for collect_config {collect_config_id}: {e}", exc_info=True
                )
                raise

        if not config_obj or not info_obj:
            raise ValueError(
                f"Cannot create PluginVersionHistory: config or info data missing for collect_config {collect_config_id}"
            )
        ...
        try:
            ...
            logger.info(
                f"Created PluginVersionHistory (id={version_obj.id}) for plugin {plugin_id} v{config_version}.{info_version}"
            )
            return version_obj.id
        except Exception as e:
            logger.error(
                f"Failed to create PluginVersionHistory for collect_config {collect_config_id}: {e}", exc_info=True
            )
            raise
...
class CustomTSTableImporter(BaseImporter, RelationImportMixin):
    ...
    注意：CustomTSTable 的主键是 time_series_group_id，且与 TimeSeriesGroup 共享
    """

    resource_type = "custom_ts_table"
    model_class = CustomTSTable

    def _get_primary_key_value(self, obj: CustomTSTable) -> int:
        """
        获取对象的主键值
    ...
    def _check_conflict(self, data: dict):
        ...
        if original_ts_group_id is not None:
            try:
                ...
                logger.debug(
                    f"Found existing {self.resource_type} by time_series_group_id: {existing.time_series_group_id}"
                )
                return existing
            except self.model_class.DoesNotExist:
                pass
        ...
        if name and bk_biz_id is not None:
            try:
                existing = self.model_class.objects.filter(name=name, bk_biz_id=bk_biz_id).first()
                if existing:
                ...
                try:
                    ...
                    if field_name:
                        existing_field = CustomTSField.objects.filter(
                            time_series_group_id=new_id, name=field_name
                        ).first()
...
class CustomTSGroupImporter(BaseImporter):
    ...
    def _check_conflict(self, data: dict):
        ...
        if name and bk_biz_id is not None:
            try:
                existing = self.model_class.objects.get(time_series_group_name=name, bk_biz_id=bk_biz_id)
                logger.debug(f"Found existing {self.resource_type} by name+biz_id: {existing.time_series_group_id}")
                return existing
            except self.model_class.DoesNotExist:
                pass
    ...
    def _create_object(self, data: dict):
        ...
        try:
            ...
            error_msg = f"Failed to create {self.resource_type}: {e}. Data keys: {list(import_data.keys())}"
            logger.error(error_msg)
            raise
