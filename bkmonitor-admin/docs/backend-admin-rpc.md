...
返回包含 `query` 回显、`space_route`、`data_label_routes`、`result_table_details`。其中 `space_route.items[].filter_groups` 会把每个 filter object 解析为 AND 条件组，多个组表示 OR；同时保留原始 `filters` 和 `raw`。接口只使用明确 field 的 `hget/hmget`，不做 `hgetall` 全量扫描。路由之间的包含关系和字段存在性检查由前端基于返回数据完成。
